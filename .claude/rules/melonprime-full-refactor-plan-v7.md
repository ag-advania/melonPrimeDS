...
|---|---|---|---|---|
| 0 | ベースライン固定 + Windows ソーク依頼 | 未着手 | — | |
| 1 | 小粒掃除（A1/A2 / API スイープ / skills 整理） | 未着手 | — | |
| 2 | HUD エディタ側パネルのテーブル駆動化（本丸） | 未着手 | — | |
| 3 | 節化・命名・ドキュメント昇格 | 未着手 | — | |
| 4 | 計測ゲート付き perf 残債（Windows 基準値待ち） | 凍結 | — | 基準値未着なら「未計測のため見送り」で閉じる |
| 5 | ドキュメント統合・クローズ処理 | 未着手 | — | |
| 6 | スモーク + CI 一括消化（最後） | 未着手 | — | ユーザー実機依存項目を含む |
