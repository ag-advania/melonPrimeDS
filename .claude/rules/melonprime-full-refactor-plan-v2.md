...
| 2 | HUDプロパティ・スキーマ統一（本丸） | 2a/2b完了（2c以降未着手） | 2026-06-11 | 2a: `.claude/skills/generate-hud-prop-schema.py` を新設し、`MelonPrimeHudPropSchema.inc` と `notes/MelonPrimeHudPropSchemaPhase2a.md` を生成。Schema rows 549、dialog/edit/side/runtime = 539/208/344/479、missing defaults 0、type drift 0、range drift 4。2b: `Config.cpp` の `Metroid.Visual.*` default 手書き行を型別 schema macro（Int/Bool/String/Double）へ置換。監査スクリプトは schema-aware に更新。`audit-config-defaults.ps1` PASS、HUD parity は Phase 0 件数と一致、MinGW configure build 成功。`Config.cpp` 内 `Instance*.Metroid.Visual.*` default literal は0件。 |
