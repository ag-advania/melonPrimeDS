...
| 2 | HUDプロパティ・スキーマ統一（本丸） | 2a完了（生成のみ / 消費側未着手） | 2026-06-11 | `.claude/skills/generate-hud-prop-schema.py` を新設し、`MelonPrimeHudPropSchema.inc` と `notes/MelonPrimeHudPropSchemaPhase2a.md` を生成。Schema rows 549、dialog/edit/side/runtime = 539/208/344/479、missing defaults 0、type drift 0、range drift 4（`BtmOverlaySrcRadius`, `HudMatchStatusLabelOfsX/Y`, `HudWeaponInventoryHighlightThickness`）。`audit-config-defaults.ps1` PASS、HUD parity は Phase 0 件数と一致、MinGW configure build 成功（Ninja no-op）。既存コードはまだ schema を include していないため挙動変更なし。 |
