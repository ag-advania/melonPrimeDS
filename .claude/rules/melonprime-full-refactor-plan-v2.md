...
| 2 | HUDプロパティ・スキーマ統一（本丸） | 2a/2b/2c完了、2d進行中 | 2026-06-11 | 2a: `.claude/skills/generate-hud-prop-schema.py` を新設し、`MelonPrimeHudPropSchema.inc` と `notes/MelonPrimeHudPropSchemaPhase2a.md` を生成。Schema rows 549、dialog/edit/side/runtime = 539/208/344/479、missing defaults 0、type drift 0、range drift 4。2b: `Config.cpp` の `Metroid.Visual.*` default 手書き行を型別 schema macro（Int/Bool/String/Double）へ置換。2c: schema に `MP_HUD_PROP_KEY_*` を追加し、`MelonPrimeInputConfigHudDialogProps.inc` へ `HudWidgetProp kSec*` を生成。`InputConfig.cpp` の dialog table 手書き key literal を include 参照へ置換。2d: `MelonPrimeHudConfigOnScreenEditProps.inc` へオンスクリーン `HudEditPropDesc` / `HudEditElemDesc` を生成し、`MelonPrimeHudConfigOnScreenDefs.inc` から手書き表を削除。`MelonPrimeHudConfigOnScreenSnapshot.inc` の固定キーは schema macro 参照へ置換。Qt side panel は exact key literal を `MP_HUD_PROP_KEY_*` へ置換（`%s` 生成フォーマット10件は既存ループ維持、完全な schema iteration 化は次作業）。監査スクリプトは schema/key-macro aware に更新。`audit-config-defaults.ps1` PASS、HUD parity は dialog/edit/side/runtime = 539/208/344/479、MinGW configure build 成功。`Config.cpp` 内 `Instance*.Metroid.Visual.*` default literal は0件、`OnScreenDefs` / `OnScreenEditProps` / `Snapshot` 内 `Metroid.Visual.*` literal は0件、`src/frontend/qt_sdl` の `Metroid.Visual.*` literal は1,184件。 |
