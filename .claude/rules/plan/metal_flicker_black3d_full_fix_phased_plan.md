...
| 4 | hires（MetalRenderer2D + RenderScreen + capture + MetalTexture 出力復帰） | 進行中（4a scaffold / 機械検証済み） | 2026-07-10 | `GPU2D_Metal.{h,mm}` を追加し、Metal 有効ビルドだけで Engine A/B 用の非可視 `MetalRenderer2D` ミラーを初期化する足場を作成。`OutputTex2D` 相当の BGRA8 render-target/ shader-read texture に加え、GL の `OBJLayerTex`/`OBJDepthTex` 相当として 2-layer RGBA8 OBJ layer target と Depth32 target を scale/device に追従して確保するようにした。BG text/affine/OBJ/window/blend/final `RenderScreen` は未実装。Phase 4e の制約どおり `Rend2D_A/B` は soft のまま維持し、可視出力は Phase 2/3 の CPU 合成 `GetOutput()` を継続。`build_metal_test.command` / `cmake --build build-mac --parallel 4` / default strings 検査 / config・inc・literal・scatter 監査 / `git diff --check` PASS。 |
