...
| 3 | native 3D パリティ（clear plane / 半透明 / shadow / toon / fog / edge） | 進行中（3-1/3-2 機械検証済み） | 2026-07-10 | 3-1 部分: plain clear plane の color/alpha に加え depth clear を `RenderClearAttr2` 由来の DS clear Z に合わせた。clear bitmap / polyID / fog flag は未完。3-2: opaque pass の `unordered_map` 全体 coalesce を廃止し、GL の `RenderPolygonBatch` と同じ隣接 key run（WBuffer / texture / TexRepeat）で提出順を保持。`build_metal_test.command` / `cmake --build build-mac --parallel 4` PASS。標準監査と ROM A/B diff は次サブフェーズ後に再実施予定。 |
