...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 native raster bootstrap, Phase 7.2 clear plane, and Phase 7.3 clear bitmap implemented; Windows build/runtime pending | `pending Phase 7.3 commit` | Windows Vulkan build pending | clear-bitmap integer texture upload, repeat/offset draw, MRT/depth write and readback harness pending | Windows UCRT64 package | ROM still uses Software; vertex upload and DS polygon raster remain pending; see `tests/vulkan/reference/phase7-clear-bitmap-verification.md` | Revert the Phase 7.3 commit |
