...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 through Phase 7.8 plus Phase 7.9A contract and Phase 7.9B shader ABI implemented; Windows build/runtime pending | `pending Phase 7.9B commit` | Windows Vulkan build pending | toon/highlight shader ABI harness pending | Windows UCRT64 package | ROM still uses Software; toon/highlight GPU draw, texture cache, fog/edge and GetLine remain pending | Revert the Phase 7.9B commit |
