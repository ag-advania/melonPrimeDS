...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 native raster bootstrap through Phase 7.7 untextured translucent blend/depth/stencil implemented; Windows build/runtime pending | `pending Phase 7.7 commit` | Windows Vulkan build pending | translucent same-polyID suppression, different-polyID blend, depth-write, W-buffer and fog-mask harness pending | Windows UCRT64 package | ROM still uses Software; textures, shadow, toon/highlight, fog/edge and GetLine remain pending; see `tests/vulkan/reference/phase7-translucent-pipeline-verification.md` | Revert the Phase 7.7 commit |
