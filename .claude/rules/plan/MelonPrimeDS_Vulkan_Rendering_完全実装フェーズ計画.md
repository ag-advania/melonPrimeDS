...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 through Phase 7.9D and Phase 7.10A texture-sampling bootstrap implemented; Windows build/runtime pending | `pending Phase 7.10A commit` | Windows Vulkan build pending | integer texture upload, nearest clamp/repeat/mirror and combiner GPU readback pending | Windows UCRT64 package | ROM still uses Software; polygon texture descriptors, texture cache, capture textures, fog/edge and GetLine remain pending | Revert the Phase 7.10A commit |
