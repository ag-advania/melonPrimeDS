...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 through Phase 7.9D, Phase 7.10A texture sampling, and Phase 7.10B textured polygon GPU draw implemented; Windows build/runtime pending | `pending Phase 7.10B commit` | Windows Vulkan build pending | integer texture upload, sampler addressing, modulate/decal/toon/highlight opaque/translucent draw readback pending | Windows UCRT64 package | ROM still uses Software; texture-cache integration, edge/fog and GetLine remain pending | Revert the Phase 7.10B commit |
