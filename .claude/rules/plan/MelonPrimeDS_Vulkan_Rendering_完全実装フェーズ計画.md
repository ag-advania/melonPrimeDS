...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 native raster bootstrap, Phase 7.2 clear plane, Phase 7.3 clear bitmap, Phase 7.4 packed vertex upload, and Phase 7.5 adjacent-only polygon batching implemented; Windows build/runtime pending | `pending Phase 7.5 commit` | Windows Vulkan build pending | pipeline/texture key construction, source-order boundaries and device-local batch readback harness pending | Windows UCRT64 package | ROM still uses Software; native opaque polygon draw remains pending; see `tests/vulkan/reference/phase7-polygon-batch-verification.md` | Revert the Phase 7.5 commit |
