...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 native raster bootstrap, Phase 7.2 clear plane, Phase 7.3 clear bitmap, Phase 7.4 packed vertex upload, Phase 7.5 adjacent-only polygon batching, and Phase 7.6 untextured opaque draw bootstrap implemented; Windows build/runtime pending | `pending Phase 7.6 commit` | Windows Vulkan build pending | Z/W, LESS/LEQUAL, alpha reject, stencil polyID and MRT readback harness pending | Windows UCRT64 package | ROM still uses Software; texture, translucent, shadow and Renderer3D integration remain pending; see `tests/vulkan/reference/phase7-opaque-pipeline-verification.md` | Revert the Phase 7.6 commit |
