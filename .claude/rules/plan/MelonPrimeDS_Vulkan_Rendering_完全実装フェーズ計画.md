...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 native raster bootstrap, Phase 7.2 clear plane, Phase 7.3 clear bitmap, and Phase 7.4 packed vertex upload implemented; Windows build/runtime pending | `pending Phase 7.4 commit` | Windows Vulkan build pending | vertex/index/edge/polygon device-local upload and exact readback harness pending | Windows UCRT64 package | ROM still uses Software; polygon pipeline keys and DS polygon raster remain pending; see `tests/vulkan/reference/phase7-vertex-upload-verification.md` | Revert the Phase 7.4 commit |
