...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 native raster bootstrap through Phase 7.8 untextured two-stage shadow mask/visible-shadow pipeline implemented; Windows build/runtime pending | `pending Phase 7.8 commit` | Windows Vulkan build pending | shadow depth-fail bit7, lower polyID preservation, self-shadow rejection, visible blend and W-buffer harness pending | Windows UCRT64 package | ROM still uses Software; textures, toon/highlight, final fog/edge and GetLine remain pending; see `tests/vulkan/reference/phase7-shadow-pipeline-verification.md` | Revert the Phase 7.8 commit |
