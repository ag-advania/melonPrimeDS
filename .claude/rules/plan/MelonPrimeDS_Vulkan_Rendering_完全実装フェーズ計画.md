...
| 8 — Vulkan texture cache/capture lifecycle | Phase 8.1 texture-key, individual-image residency, 9-sampler table, descriptor cache and dirty invalidation bootstrap implemented; Windows build/runtime pending | `pending Phase 8.1 commit` | Windows Vulkan build pending | cache plan and GPU sampling/readback pending | Windows UCRT64 package | display capture, upload ring, savestate, reset and renderer integration remain pending | Revert the Phase 8.1 commit |
