...
| 12 — UI/settings/presets/localization | Implemented as a capability-aware UI subsystem candidate: dynamic Vulkan/Vulkan Compute choices, concrete device and ROM-acceptance reasons, hardware-setting key migration with legacy dual-write, complete raster/compute presets, option enable matrix, macOS MoltenVK gate, unknown-ID normalization and all selectable language-ID coverage; Windows/runtime acceptance pending | `pending Phase 12 completion commit` | Windows Vulkan build pending | Phase 12 UI contract and Phase 8 through 11 regressions pending | Windows UCRT64 package | ROM-visible Vulkan choices remain disabled until pixel-parity acceptance; developer override is non-persistent | Revert the Phase 12 completion commit |
| 13 — VSync/frame pacing/device loss/pipeline cache | Implemented: configured VSync interval policy, latest-completed-frame drop during Fast Forward/Slow Motion, one-pending-acquire guard for Qt FIFO presentation, actual surface present-mode query and FIFO/low-latency swapchain creation harness, process-local device-loss fallback that preserves Config, identity-bound atomic VkPipelineCache persistence, cold/warm validation and pipeline prewarm; Windows GPU/runtime acceptance pending | `pending Phase 13 completion commit` | Windows Vulkan build pending | Phase 13 harness plus Phase 8 through 12 regressions pending | Windows UCRT64 package | QVulkanWindow runtime remains FIFO and uses latest-frame drop; direct present-mode swapchains are validated by harness | Revert the Phase 13 completion commit |
| 14～16 | Not started | — | — | — | — | Phase順に実施 | Phase単位でrevert |
