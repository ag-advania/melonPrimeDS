...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 through Phase 7.8 plus Phase 7.9A toon/highlight contract implemented; Windows build/runtime pending | `pending Phase 7.9A commit` | Windows Vulkan build pending | toon-table/reference harness pending | Windows UCRT64 package | ROM still uses Software; toon/highlight GPU draw, texture cache, fog/edge and GetLine remain pending | Revert the Phase 7.9A commit |
