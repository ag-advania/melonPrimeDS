...
| 8 — Vulkan texture cache/capture lifecycle | Phase 8.1 residency, 9-sampler and descriptor-cache bootstrap plus Phase 8.2 all-format RGB6A5 decode, footprint hash and 512-byte dirty-page filtering implemented; Windows build/runtime pending | `pending Phase 8.2 commit` | Windows Vulkan build pending | seven-format decode and cache regression harnesses pending | Windows UCRT64 package | upload ring, display capture, savestate, reset and ROM integration remain pending | Revert the Phase 8.2 commit |
