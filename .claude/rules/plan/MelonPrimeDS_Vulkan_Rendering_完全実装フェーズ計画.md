...
| 8 — Vulkan texture cache/capture lifecycle | Phase 8.1 residency, Phase 8.2 all-format RGB6A5 decode/dirty-page filtering and Phase 8.3 persistent staging upload ring with aligned non-coherent flush and fence-gated wrap reuse implemented; Windows build/runtime pending | `pending Phase 8.3 commit` | Windows Vulkan build pending | upload-ring, decode and cache regression harnesses pending | Windows UCRT64 package | timeline semaphore, display capture, savestate, reset and ROM integration remain pending | Revert the Phase 8.3 commit |
