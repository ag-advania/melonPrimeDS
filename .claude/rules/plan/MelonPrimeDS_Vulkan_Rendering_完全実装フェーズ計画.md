...
| 7 — Vulkan Classic 3D correctness baseline | Phase 7.1 through Phase 7.8, Phase 7.9A contract, Phase 7.9B shader ABI, Phase 7.9C descriptor boundary, and Phase 7.9D GPU draw implemented; Windows build/runtime pending | `pending Phase 7.9D commit` | Windows Vulkan build pending | real descriptor allocation/bind plus opaque/translucent toon/highlight readback pending | Windows UCRT64 package | ROM still uses Software; texture sampling, fog/edge and GetLine remain pending | Revert the Phase 7.9D commit |
