...
- macOS 配布時の署名: アプリは未署名（`codesign: not signed at all` 実測）。開発中は
  GCMouse化で実害を回避済みだが、配布物は最低でも安定した ad-hoc/Developer ID 署名が必要
  （未署名だと Gatekeeper 隔離で起動体験が悪く、ユーザー側のTCC許可も再取得になる）
...
| # | 作業 |
|---|---|
| 5-1 | CMake に `MELONPRIME_MAC_CODESIGN_IDENTITY` 署名フック（未指定なら ad-hoc `-s -` で最低限の署名。Gatekeeper 完全対応の Developer ID + notarization は**要確認**: Apple Developer Program 加入状況次第） |
| 5-2 | `build-macos.yml` のアーティファクト整備（.app を zip/dmg 化、`MELONPRIME_ENABLE_DEVELOPER_FEATURES=OFF` の配布構成であることを確認） |
| 5-3 | `build-ubuntu.yml` のアーティファクト整備（AppImage or tar.gz — 形式は**要確認**） |
...
**残存:**
5. Apple Developer Program 加入状況（Phase 5-1 の署名レベル: ad-hoc か Developer ID + notarization か）
6. Linux 配布形式（Phase 5-3: AppImage / tar.gz / deb）
