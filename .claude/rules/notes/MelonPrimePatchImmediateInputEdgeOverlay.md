...
| Direct Alt-Form Transform | Morph / AltForm (変形・解除) | `ImmediateTransformGateHook` で native TransformRequest 経路へ redirect |
...
### Morph Ball Boost との相互作用

`HK_MetroidHoldMorphBallBoost` は overlay の binding table 対象ではない。
通常フレーム側の `HandleMorphBallBoost()` が Samus alt-form 中に `INPUT_R` を
hardware input として合成する。

Immediate Input Edge Overlay が有効な場合、control preset によっては
`zoomMask` が `INPUT_R` と同じ bit になる。この時 overlay が
`IB_ZOOM` だけを見て `input.current` を置換すると、boost 用に合成された
`INPUT_R` が消えて Morph Ball Boost が発動しない。

対処として、`HandleMorphBallBoost()` が `INPUT_R` を管理したフレームでは
`m_immediateOverlayPreserveMask` に `INPUT_R` bit を立てる。

overlay 側では:

```
preserveMask = m_immediateOverlayPreserveMask & managedMask
overlayMask  = managedMask & ~preserveMask
```

として、`preserveMask` に含まれる bit は `current / pressed / released` の
game-polled 結果をそのまま残す。

これにより、boost 側が合成した hardware input を overlay が上書きしない。
overlay が `INPUT_R` を管理していない preset では `preserveMask == 0` になり、
通常どおり game poll 結果を残す。

### Direct Alt-Form Transform 対象 (旧方式メモ)
...
旧方式では `Consts::MORPH_SYNTHETIC_BINDING = 0x00040004` を書き込んでいたが、
現在の Direct Alt-Form Transform は `ImmediateTransformGateHook` 側で
native TransformRequest 経路へ redirect する。
...
```
managedMask = movLMask | movRMask | movFMask | movBMask   (ImmediateInputEdgeOverlay)
            | fireMask | jumpMask | zoomMask              (ImmediateInputEdgeOverlay)
```

現在の Transform は `ImmediateTransformGateHook` 側に移っているため、
ImmediateInputEdgeOverlay 本体では morphMask を管理しない。

Morph Ball Boost は managedMask には追加しない。
`HandleMorphBallBoost()` が `m_immediateOverlayPreserveMask` に `INPUT_R` を立て、
`INPUT_R` が既に managedMask に含まれている時だけ overlay replacement から外す。
これにより、game-polled boost 押下を overlay が消さないようにする。
