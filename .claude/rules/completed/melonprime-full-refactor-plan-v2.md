...
|---|---|---|---|---|
| 0 | ベースライン + キー監査ツール | ビルド検証済み（手動スモークは継続回帰項目） | 2026-06-11 | `wc -l` 計測、`"Metroid.*"` リテラル数、exeサイズを記録。`.claude/skills/audit-hud-key-parity.ps1` を新設し、Config defaults / dialog / edit descriptors / side panel / runtime load のキー集合差分を出力可能にした。Phase 6 で、手動スモーク未実施は本リファクタの未完ブロッカーではなく継続回帰項目として扱う方針に整理。 |
...
| 8 | ドキュメント + 最終計測 | 完了（ビルド未検証） | 2026-06-11 | `melonprime-refactoring.md` に `Structural Refactor V2 2026-06` 節を追加。`repo-architecture.md` に HUD schema ownership と新規 HUD property 追加手順を追加。`melonprime-patch-system.md` に shared hook-site tables 節を追加。`completed/melonprime-full-refactor-plan-v2.md` を新設し、`CLAUDE.md` / rules README から V2 計画を active index から completed index へ移動。最終計測: quoted `"Metroid.*"` literals under `src/frontend/qt_sdl` = 1,040（Phase 0 1,802 から -762。549件は `MelonPrimeHudPropSchema.inc` の canonical owner literals）、`MelonPrimeInputConfig.cpp` = 995行、HUD schema rows = 549、HUD parity missing defaults = 0。ReadLints green。Shell runner が exit status を返さないため build / exe size は未検証。 |
...
- HUDキー面間監査（`.claude/skills/audit-hud-key-parity.ps1 -MaxList 20`）:
...
- ビルド基準: `.\.claude\skills\build-mingw.bat --tail 80` 成功（configure + vcpkg確認 + Ninja no-op、約5秒。クリーン再コンパイルは未強制）
