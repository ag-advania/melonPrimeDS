...
[V4](completed/melonprime-full-refactor-plan-v4.md) /
[V5](completed/melonprime-full-refactor-plan-v5.md) の後継。
...
- `MELONPRIME_PERF` の消費箇所は `MelonPrimePerfProbe.h` + `EmuThread.cpp` の区間プローブ
  ...
  **基盤は完成**。macOS / Linux VM で初回 ROM 10 分ソークを取得
  ...
  端末出力から復元）。
  Windows は未計測。
- V5 進捗表の pending 項目のうち **mac / Linux VM 基準値は §8 へ反映可能**。Win・S21/S22・V5 確定は pending。
...
- `MelonPrime.cpp` 976 行 / `MelonPrime.h` 714 行。`RunFrameHook()` は **243 行**
  ...
  1 関数で司る。
- [Post-V4 ロードマップ](notes/melonprime-highres-fonts-v3-refactor-roadmap.md) Phase 1 と
...
| 対象 | 証拠 |
|---|---|
| `MelonPrimeHudRenderCrosshairFx.inc`（462 行）が正典の所有表に無い | [melonprime-refactoring.md](melonprime-refactoring.md) §12/§18.2 の HUD 断片表に行が無い。include 親は `HudRenderDraw.inc:800`（`.inc` が `.inc` を include するネスト断片の初出だが、所有検査は PASS） |
...
6. 検証 3 系統: Windows CI / mac ローカル / Linux VM（[linux-vm-build.md](linux-vm-build.md)、
   マウス統合オフ必須）。ビルドは各 OS の正規手順（[build.md](build.md)）。
...
| 項目 | 内容 |
|---|---|
| 0-1 | **計測手順書の整備**: 3 プラットフォーム別に「dev ビルド → `MELONPRIME_PERF=1` 起動 → in-game 10 分ソーク（対戦 or bot 相手）→ `summarize-melonprime-perf.py` 集計 → 記録」の 1 ページ手順を `.claude/skills/perf-baseline-procedure.md` としてチェックイン。AI が準備できる部分（ビルド、ログ解析、表の更新）と、ユーザー実機が必要な部分（ROM 起動・プレイ）を明確に分離する |
...
- golden ファイル（ハッシュのみ）を `src/frontend/qt_sdl/tests/`（または `.claude/skills/` 配下の
  データとして）チェックインし、`.claude/skills/run-hud-golden.{sh,ps1}` で「実行 → diff」を 1 コマンド化
...
| 3-4 | edit mode は対象外（毎フレーム全要素 bounds が必要）。invalidation owner を [melonprime-performance.md](melonprime-performance.md) の台帳に**同コミットで**追記 |
...
1. [melonprime-refactoring.md](melonprime-refactoring.md) に「Structural Refactor V6 / Round 11 — Measured」節を追記（Phase 3/5 の実測値表）
2. [melonprime-performance.md](melonprime-performance.md) の per-frame syscall 予算表と invalidation 台帳を V6 後の姿に更新。**フレームタイム基準値表（3 プラットフォーム）を正典に固定** — 以後の性能 PR は基準値との比較添付が必須というラチェット
...
|---|---|---|---|---|
| 0 | V5 検証負債の完済（基準値・S21/S22・V5 確定） | 一部完了 / Windows ソーク待ち | 2026-07-04 | 0-1 完了: `.claude/skills/perf-baseline-procedure.md` 等。**0-2 macOS 完了**（`macos-perf-20260704-082454`、8192 frames、p50=16.689、draw p50=0.732ms、GCMouse）。**0-2 Linux VM 完了**（`linux-vm-perf-20260704-085743`、6497 frames、p50=49.216、draw p50=16.929ms、XInput2 RawMotion、llvmpipe VM）。`collect-perf-baseline.sh` に書き込み不可時の fallback を追加。0-3/0-4/0-5 と Windows 基準値は pending |
...
| 3 | HUD element cache / layer 分離（本丸 A・二重ゲート） | 未着手 | — | mac 基準値: draw p50=0.732ms / run p50=9.809ms / `CustomHud_Render` p50=219.6µs → **ゲート(1) は現時点で不採用方向**（mac）。Linux VM: draw p50=16.929ms だが **llvmpipe ソフトウェア GL の VM 固有** — ネイティブ Linux GPU 再計測前に Phase 3 着手不可 |
...
| 7 | ドキュメント + ラチェット固定 | 固定可能分完了 / 実測待ち | 2026-07-04 | scatter 22/22 にラチェット、Windows/Ubuntu CI 更新、HUD golden 運用ルールを build.md に追記、performance/refactoring/repo docs に V6 現況と未完了ゲートを記録。mac / Linux VM 基準値 §8 反映済み。Windows 表・completed 移動は pending |
...
|---|---:|---:|---:|---:|---:|---|
| macOS | 16.689 | 22.829 | 24.929 | 173.172 | 0.732 | 2026-07-04 soak、`MELONPRIME_PERF=1` dev build、GCMouse、8192 frames shutdown。max 173ms は単発スパイク。区間 p50 (ms): run 9.809 / sleep 5.785 / spin 0.366 / input 0.005 |
| Windows | 未取得 | 未取得 | 未取得 | 未取得 | 未取得 | |
| Linux VM | 49.216 | 103.792 | 141.452 | 290.728 | 16.929 | **VM 上実行のため FPS はかなり悪い**（p50 ≒ 20 fps 相当。VirtualBox + llvmpipe ソフト GL — ネイティブ Linux 実機とは不可比）。2026-07-04 soak、XInput2 RawMotion、6497 frames。区間 p50 (ms): run 34.024 / draw 16.929。ログは tee 失敗のため端末出力から復元 |
...
| `OsdColor_ApplyOnce` 実 write / 分 | 0.1 | — | 0.4 | mac: apply 3398.3/分。Linux VM: apply 1026.6/分 |
