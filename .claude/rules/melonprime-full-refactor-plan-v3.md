...
| 対象 | 証拠 |
|---|---|
| `Metroid.Aim.Enable.InstantAimFollow` migration | [InputConfig.cpp:418](../../src/frontend/qt_sdl/InputConfig/MelonPrimeInputConfig.cpp#L418) / [InputConfigConfig.cpp:120](../../src/frontend/qt_sdl/InputConfig/MelonPrimeInputConfigConfig.cpp#L120) / runtime 1箇所に「planned removal after the next release」バナー。V2 Phase 1 で期日明記済み → **削除の実行判断が必要**（リリースが挟まったかを確認の上） |
...
| # | 作業 |
|---|---|
| 4-1 | **migration 台帳の作成**: 全 migration コード（InstantAimFollow 3箇所、EmuInstance.cpp のアンカー migration、`HudFontSize`、ZoomMethod/WeaponSwitchMethod/BipedFireMethod の enum 変換）を1つの表に棚卸しし、本ファイルまたは notes/ に記録。層跨ぎの物理移動はしない（V1 4-4 の判断を維持） |
