...
| 2 | パッチサブシステム統一 | 2b実装完了（ビルド検証待ち）/ 2a未着手 | 2026-06-10 | 2b: `MelonPrimePatchRegistry.h/.cpp` 新設（11モジュールを site マスク + restore フラグで駆動）。MelonPrime.cpp の Apply×16 / Restore×16 / Reset×33 をレジストリ4API呼びに置換（ARM9Hook / CustomHud / per-frame OsdColor は手動のまま）。挙動差はゲーム離脱時の OsdColor 復元順（最後→6件中先頭。書込アドレス互いに素で安全）と同呼び出しの `#ifdef MELONPRIME_DS` ガード内移動のみ。patch-system.md 更新済み |
| 3 | テーブル単一情報源化 | 未着手 | — | — |
| 4 | ライフサイクル整理 | 未着手 | — | — |
| 5 | UI/設定データ駆動化 | 未着手 | — | — |
| 6 | HUD断片整合 | 未着手 | — | — |
