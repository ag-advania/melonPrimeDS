...
- [MelonPrimeZoomInputMethods.md](notes/MelonPrimeZoomInputMethods.md) の config 値表（`0=New preset binding / 1=Legacy fixed R`）が
...
| 5 | UI/設定データ駆動化 | 5a+5c実装完了（ビルド検証待ち）/ 5b未着手 | 2026-06-12 | 5a: `MelonPrimeLocalization.h`（1212行）から翻訳テーブル本体（`kTranslations[]`/`kObjectTextTranslations[]`、`Translation`/`ObjectTextTranslation` 構造体定義込み）と全関数定義を新規 `MelonPrimeLocalization.cpp`（1170行）へ移動。ヘッダは69行（`#pragma once`、Qt include 6本、トリビアルな inline ヘルパー6個+定数2個、関数宣言18個のみ）に縮小。データ配列は `inline` を外し非公開 `constexpr`（.cpp内では暗黙に internal linkage）。新旧の構造体定義・配列・関数本体はHEADと全行byte-exact一致を確認済み（784/784データ行、354/354関数行）。CMakeLists.txt の `SOURCES_QT_SDL` に `MelonPrimeLocalization.cpp` を追加（`MelonPrime.cpp` の直後）。消費側4ファイルは無変更（同じヘッダ・同じシグネチャをinclude）。g++ -fsyntax-only でヘッダ単体・cpp単体とも構文確認済み。5c: `repo-architecture.md` の "GetXxx Default Coverage Audit" に埋め込まれていた監査スクリプトを `.claude/skills/audit-config-defaults.ps1` として新規チェックイン（90行）。埋め込み版が抱えていた2つの実行不能バグ（PowerShell二重引用符内の `\"` は無効エスケープでParserError／`rg` がこの環境のPATHに存在しない）を修正し、`Get-ChildItem`+`[regex]::Matches` ベースの純PowerShell実装に書き換え。`repo-architecture.md` の当該節は説明文を残しつつ埋め込みコードを新スクリプトへのポインタに置換。実行結果は8セクション全て空、`PASS:` メッセージ、終了コード0（クリーン）。NO build / NO commit。5b（非HUD設定のバインディングテーブル化）は未着手。 |
| 6 | HUD断片整合 | 未着手 | — | — |
| 7 | 統合点集約（任意） | 未着手 | — | — |
| 8 | ドキュメント整備 | 未着手 | — | — |
