...
def parse_defaults() -> tuple[list[str], dict[str, Prop]]:
    text = strip_comments(read(CONFIG_CPP))
    props: dict[str, Prop] = {}
    order: list[str] = []
    state = ""
    type_re = {
...
def write_schema(order: list[str], props: dict[str, Prop], path: Path) -> None:
    ...
    lines: list[str] = [
        ...
        "// Phase 2a schema seed: one row per Config.cpp Metroid.Visual.* default.",
    ...
    for key in order:
        ...
        row_lines.append(
            "    X("
            f"{ident}, {cpp_quote(key)}, {prop.cfg_type}, {prop.default_expr}, "
    ...
    for index, row in enumerate(row_lines):
        suffix = " \\" if index + 1 < len(row_lines) else ""
