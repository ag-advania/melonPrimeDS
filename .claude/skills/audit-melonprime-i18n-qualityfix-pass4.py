...
COMMON_ALLOW = {
    "ON","OFF","OK","OpenGL","VSync","ARM9","ARM7","BIOS","DS","DSi","JIT","HUD","OSD","FPS","ROM","RAM","CPU","GPU","SD","MAC","IP",
...
def nontech_tokens(s: str) -> list[str]:
    out = []
    for t in english_tokens(s):
        clean = t.strip(".-")
        if clean in COMMON_ALLOW or re.fullmatch(r'[A-Z]{1,4}', clean) or re.fullmatch(r'[A-Z]{1,4}[0-9.]+', clean) or re.fullmatch(r'[a-z]?\d+(\.\d+)?', clean):
            continue
        out.append(t)
    return out
        ...
        for lang in TARGET_LANGS:
            ...
            for m in block_re.finditer(text):
                ...
                if lang == "Filipino" and val.startswith("Paliwanag — "):
                    fallback_sentence_hits += 1
                if lang == "Swahili" and val.startswith("Maelezo — "):
                    fallback_sentence_hits += 1
                if name != "object" and lang in NONLATIN:
                    toks = nontech_tokens(val)
                    if len(toks) >= 5 and RATIO[lang](val) < 0.20:
                        heavy_hits += 1
            if prefix_hits:
                print(f"[FAIL] {name}: {lang} prefix fallback remains: {prefix_hits}")
                failed = True
            if fallback_sentence_hits:
                print(f"[FAIL] {name}: {lang} fallback sentence remains: {fallback_sentence_hits}")
                failed = True
            if heavy_hits:
                print(f"[FAIL] {name}: {lang} non-Latin English-heavy rows remain: {heavy_hits}")
                failed = True
