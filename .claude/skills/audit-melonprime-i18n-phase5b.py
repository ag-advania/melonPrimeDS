...
DECISIONS_CSV = Path("i18n_quality_phase5b/phase5b_final_decisions.csv")
...
def main() -> int:
    ...
    with DECISIONS_CSV.open("r", encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            dec = row.get("phase5b_final_decision", "")
            if dec in {"TRANSLATE_UI", "NEEDS_CONTEXT", ""}:
                unresolved.append(row)
            if dec == "APPLIED_COMFORT_TRANSLATION":
                comfort += 1

    if unresolved:
        print(f"[FAIL] unresolved Phase 5B rows: {len(unresolved)}")
        failed = True
    else:
        print("[PASS] Phase 5B final decisions have no TRANSLATE_UI/NEEDS_CONTEXT rows")
    print(f"[INFO] APPLIED_COMFORT_TRANSLATION rows: {comfort}")
