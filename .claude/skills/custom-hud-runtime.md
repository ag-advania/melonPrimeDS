...
| OPT-ZOOM1 | Zoom crosshair state and display smoothing are keyed by `NDS::NumFrames`: `UpdateCrosshairZoomAmountForGameFrame()` polls RAM once per emulated game frame, and `s_chDisplayZoom` advances once per game frame while repeated 120/240Hz draws reuse the cached amount | Prevents high-refresh presentation from accelerating 2-frame zoom/debounce transitions and avoids per-draw zoom RAM polling | Per game frame |
