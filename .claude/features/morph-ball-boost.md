...
Samusではない、またはShiftが押されていないフレームは早期returnし、
Morph Ball Boost由来のaim blockだけ必要に応じて解除する。
...
5. その後にマウス/スタイラスaim処理が走る。ブースト操作中はaim blockで干渉を避ける。
...
- まだブースト中でなくゲージが足りていない間は、Rを押してチャージする。
- ゲージが足りていてまだブースト中でない時は、Rを解放してブーストを発動させる。
- ゲーム側が `isBoosting != 0` になったら、次のサイクルに備えてRを押す側へ戻す。
...
「チャージ中は押す、発動タイミングで離す、ブースト中はまた押す」というサイクルを作る。

## Aimとタッチ入力の扱い
Morph Ball Boost中は `AIMBLK_MORPHBALL_BOOST` を立てて、マウスaimやスタイラス入力が
ブースト操作に干渉しないようにする。

- Alt Form中にShift高速ブースト処理へ入ると、aim blockを有効化する。
- Weapon Check中でなければ `ReleaseScreen()` してタッチ入力も離す。
- Shiftを離す、Samusでない、またはブースト状態に入った時はaim blockを解除する。

`ProcessAimInputMouse()` は `m_aimBlockBits != 0` の時に早期リセットへ入り、
残留aim量やnative aim deltaをクリアしてから戻る。
