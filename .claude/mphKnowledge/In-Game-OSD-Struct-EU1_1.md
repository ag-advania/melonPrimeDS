# In-Game-OSD-Struct-EU1_1

`mphDump/EU1_1.txt` で確認したEU1.1版のin-game OSD message構造と色定義メモ。
...
EU1.0版の対応メモは [`In-Game-OSD-Struct-EU1_0.md`](In-Game-OSD-Struct-EU1_0.md)。

## EU1.1 Runtime Layout

EU1.1のOSD enqueue相当は `0204C518`。呼び出し元が `stack+0x04` に置いた色をslotの `entry+0x10` へ保存する。

EU1.1はJP1.0/KR1.0と同じ `0x9C` stride。US1.0の `0x80` strideとは違うので、US用slot直書きpatchをそのまま移植しないこと。

| Item | EU1.1 Value | Notes |
| --- | --- | --- |
| OSD enqueue / setup function | `0204C518` | EU1.0 `0204C4EC` 相当。 |
...
| OSD MSG base | `0x020E4574` | slot 1 entry base。 |
| Entry stride | `0x9C` | JP1.0/KR1.0と同じ。 |
| Font pointer source | `0x020DF020 -> 0x020CBFFC` | enqueue callerが `stack+0x0C` に置く。 |
...
```text
color_addr = 0x020E4574 + (slot - 1) * 0x9C + 0x10
```
...
| --- | --- | --- |
| 1 | `0x020E4584` | `120E4584 00003FEF` |
| 2 | `0x020E4620` | `120E4620 00003FEF` |
| 3 | `0x020E46BC` | `120E46BC 00003FEF` |
| 4 | `0x020E4758` | `120E4758 00003FEF` |
| 5 | `0x020E47F4` | `120E47F4 00003FEF` |

## Message Text Lookup

EU1.1でOSD呼び出し元が渡す `message id` は `0203C2E0` で文字列ポインタへ変換される。
...
| --- | --- | --- | --- |
| `< 0x0C` | `0x020DF0F8` | `HudMsgsCommon.bin`, `H###` | static dumpでは `count=0x0B`, table `0x02287C04`。 |
| `0x0C..0x7A` | `0x020DF0F0` | `HudMessagesSP.bin`, `H###` | single-player側ロード時に使う領域。static dumpでは未ロード。 |
| `0x7B..0x101` | `0x020DF118` | `HudMessagesMP.bin`, `H###` | static dumpでは `count=0x64`, table `0x02287D24`。 |
| `0x12C..0x131` | `0x020DF118` | `HudMessagesMP.bin`, `W###` | `id - 0x12C` を `W###` として引く。 |

EU1.1のstatic dumpでロード済みのcommon/MP tableは英語。SP tableは未ロードなので、`H066` や `H121` などSP側候補の本文はこのdumpだけでは確定しない。

OSD色調査で出てきたmessage idとEU1.1文字列:
...
| `0x0C9` | `H201` | `your octolith reset!` | `0x0212E0F8`, `0x0212E484`, or `0x0212E5EC` |
...
| `0x0CF` | `H207` | `enemy octolith reset!` | `0x0212E0F8`, `0x0212E484`, or `0x0212E5EC` |
| `0x0D3` | `H211` | `node stolen` | `0202D8B0: mov r2,#0x1F` / `0202D8EC` |
| `0x0E4` | `H228` | `HEADSHOT!` | `0x0201828C` / `0201750C` |
| `0x0E5` | `H229` | `the octolith\nhas been dropped!` | `0x0212E484` / `0212E408` |
| `0x0E6` | `H230` | `the enemy\ndropped your octolith!` | `0x0212E484` / `0212E368` |
| `0x0E7` | `H231` | `your team\ndropped the octolith!` | `0x0212E484` / `0212E368` |
| `0x0E8` | `H232` | `your octolith is missing!` | `0x0212EE4C` / `0212EDA8` |
| `0x0E9` | `H233` | `turret energy: %d` | `0x0211098C` / `021106E0` |
...
| `0x101` | `H257` | `octolith reset!` | `0x0212E0F8` or `0x0212E484` |

## Color Source Investigation

EU1.1の `0204C518` への直callは28件あり、色定義は `11 literal + 1 immediate` の12箇所に集約できる。
...
特にno-ammo系は `0202DA40-0202DA90` のブロックで、`0202DA40` が message id `0x09` をセットし、`0202DA50` が色literal `0x0202DE44 = 0x0000295F` を読む。ここがEU1.1の最小patch候補。

```text
; EU1.1: message id 0x09 / H009 / "AMMO DEPLETED!" color -> cyan
0202DE44 00007FE0

; EU1.1: same target -> pure red
0202DE44 0000001F

; EU1.1: message id 0xCC/0xCD / H204/H205 color -> cyan
02031BA8 00007FE0

; EU1.1: shared 0xCA/0xCB/0xCE notification color -> cyan
0202DE2C 00007FE0
```
...
| `0x0211098C` | `0x3FEF` | `021106E0` | `0211098C 0000CCCC` | `0xE9 = H233 = turret energy: %d`。full側追加領域。 |
| `0x0212E0F8` | `0x3FEF` | `0212E0DC` | `0212E0F8 0000CCCC` | message id `0xC9/0xCF/0x101`。 |
| `0x0212E484` | `0x3FEF` | `0212E368`, `0212E408` | `0212E484 0000CCCC` | message id `0xC9/0xE6/0xCF/0xE7` または `0x101/0xE5`。 |
| `0x0212E5EC` | `0x3FEF` | `0212E570` | `0212E5EC 0000CCCC` | message id `0xC9/0xCF`。 |
| `0x0212EE4C` | `0x3FEF` | `0212EDA8` | `0212EE4C 0000CCCC` | `0xE8 = H232 = your octolith is missing!`。 |
...
- no-ammo系だけを狙うなら `0202DE44` から試す。`02031BA8` は `progress` / `acquiring\nnode` 用。`0202DE2C` や `0201828C` は複数メッセージをまとめて変える。
...
| `021106E0` | `0xE9` | `0x0211098C` | `0x3FEF` | `0x01` | `0x00` | `H233 = turret energy: %d`。 |
| `0212E0DC` | `0xC9/0xCF/0x101` | `0x0212E0F8` | `0x3FEF` | `0x3C` | `0x01` | full側追加領域。 |
| `0212E368` | `0xC9/0xE6/0xCF/0xE7` | `0x0212E484` | `0x3FEF` | `0x3C` | `0x01` | `H201/H230/H207/H231`。 |
| `0212E408` | `0x101/0xE5` | `0x0212E484` | `0x3FEF` | `0x3C` | `0x01` | `H257/H229`。 |
| `0212E570` | `0xC9/0xCF` | `0x0212E5EC` | `0x3FEF` | `0x3C` | `0x01` | `H201/H207`。 |
| `0212EDA8` | `0xE8` | `0x0212EE4C` | `0x3FEF` | `0x01` | `0x00` | `H232 = your octolith is missing!`。 |
