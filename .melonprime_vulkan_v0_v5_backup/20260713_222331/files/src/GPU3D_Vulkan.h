...
public:
    ...
    static std::unique_ptr<VulkanRenderer3D> New(GPU3D& gpu3D) noexcept;

    explicit VulkanRenderer3D(GPU3D& gpu3D) noexcept;
    ...
    void RenderFrame() override { RenderFrame(GPU); }
    ...
    void Reset(GPU& gpu);
    void VCount144(GPU& gpu);
    void RenderFrame(GPU& gpu);
    void RestartFrame(GPU& gpu);
    ...
    [[nodiscard]] bool UsesStructured2DMetadata() const noexcept override { return false; }
    void Blit(const GPU& gpu);
    void Stop(const GPU& gpu);
