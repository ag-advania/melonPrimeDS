...
| `src/frontend/qt_sdl/VulkanReference/VulkanSurfacePresenter.h` | `VulkanSurfacePresenter.h` | Reference-majority presenter with local includes and `volk`; Android native window API is excluded. R20 accepts a borrowed `VkSurfaceKHR`, so swapchain/layout/pacing remain here while native-window and surface ownership do not. |
| `src/frontend/qt_sdl/VulkanReference/VulkanSurfacePresenter.cpp` | `VulkanSurfacePresenter.cpp` | Reference-majority surface format, present mode, extent, swapchain, layout, descriptor, and pacing implementation. R20 removes Win32 creation/destruction and consumes the surface/pixel extent supplied by the desktop host. |
