...
| `src/VulkanContext.h` | `src/VulkanContext.h` | Desktop platform adaptation: process-wide ref-counted context, desktop surface extensions, and `volk`; R4 completes platform instance/present requirements, R20 resolves a frontend-created Win32/XCB/Wayland/Metal surface to its present queue, and R24 adds persistent device-loss reporting plus centralized VkResult classification. |
| `src/VulkanR24Sync.h` | No direct Android file | Desktop-only R24 adapter: purpose-specific barriers and completion-associated retire queues. It changes neither Sapphire raster/compositor algorithms nor descriptor ABI. |
| `src/VulkanContext.cpp` | `src/VulkanContext.cpp` | Desktop platform adaptation corresponding to the header; Android surface/AHB/debug integration is intentionally not imported into core. R20 enables the XCB/Xlib/Wayland extension set on Unix desktop and retains portability enumeration for MoltenVK. |
...
| `src/frontend/qt_sdl/VulkanReference/VulkanSurfacePresenter.h` | `VulkanSurfacePresenter.h` | Reference-majority presenter with local includes and `volk`; Android native window API is excluded. R20 accepts a borrowed `VkSurfaceKHR`. R21 adds a desktop affine-layout input and immutable GPU-overlay command ABI; the pinned swapchain/background/filter/pacing structures remain the base. |
| `src/frontend/qt_sdl/VulkanReference/VulkanSurfacePresenter.cpp` | `VulkanSurfacePresenter.cpp` | Reference-majority surface format, present mode, extent, swapchain, descriptor, and pacing implementation. R20 removes native surface creation/destruction. R21 maps Qt `ScreenLayout` affine transforms directly to vertices, consumes the producer-composed Ready image without a second composition submit, and appends solid HUD/OSD plus bottom-atlas radar draws to the same render pass. |
| `src/frontend/qt_sdl/VulkanReference/VulkanSurfacePresenter.vert` | `VulkanSurfacePresenter.vert` | R21 MelonPrime adaptation over the reference vertex shader: one interpolated RGBA attribute is added for GPU-generated solid overlay quads; screen/background position and UV behavior is unchanged. |
| `src/frontend/qt_sdl/VulkanReference/VulkanSurfacePresenter.frag` | `VulkanSurfacePresenter.frag` | R21 MelonPrime adaptation over the reference compositor/presenter fragment shader: draw modes 7/8 render solid HUD/OSD and a circular radar that samples the bottom region of the already-composed two-screen image. Existing draw modes and composition/filter algorithms are unchanged. |
| `src/frontend/qt_sdl/VulkanReference/VulkanFilterMode.h` | `VulkanFilterMode.h` | Reference code. |
| `src/frontend/qt_sdl/MelonPrimeVulkanFrontendSession.h/.cpp` | `VulkanOutput.h/.cpp` consumer boundary | MelonPrime desktop adapter: R17 copies the complete engine-order snapshot from the active outer `Renderer`, applies the latched screen swap exactly once, and then performs the sole Control-ABI conversion. R18 transports source identity. R19 makes FrameQueue the sole slot owner and synchronizes VulkanOutput history plus presenter completion into distinct queue references before producer or GUI acquisition. R21 serializes dynamic overlay publication with those presenter completion checks under the same session mutex. R22 adds `beginBackendSwitch()`/`completeBackendSwitch()` with a `producerSuspended` flag and a `stagedPresenter` slot, so no frame submission, presenter (re)registration, or overlay publication can race the single-transaction outer/3D/presentation backend switch driven from `EmuThread::msg_ApplyVideoBackendSwitch`. |
| `src/frontend/qt_sdl/MelonPrimeVulkanSurfaceHost.h/.cpp/.mm` | desktop-only boundary; no Android source equivalent | R20 sole `VkSurfaceKHR` owner. It obtains documented Qt native interfaces for Win32, XCB, Wayland, and AppKit/CAMetalLayer, reports device-pixel-ratio size, assigns a process-monotonic native-window generation, and resolves the shared present queue before lending the surface to the presenter. |
| `src/frontend/qt_sdl/MelonPrimeScreenVulkan.h/.cpp` | desktop-only Qt consumer boundary | R21 GUI consumer. EmuThread publishes an immutable HUD snapshot and only queues repaint; the GUI thread owns native-surface validation, DPR/layout conversion, overlay command generation, Ready/Previous acquisition, direct swapchain present, and commit/defer. It inherits `ScreenPanel` directly and never enters the Native/QImage/QPainter presenter. |
...
## R24 synchronization/lifetime completion

- `VulkanRenderer3D` keeps the Sapphire render algorithm and retires replaced render targets only after the graphics queue-tail completion point.
- `TexcacheVulkanLoader` associates invalidated texture arrays with the last published 3D timeline value; the renderer drains them before destroying that timeline.
