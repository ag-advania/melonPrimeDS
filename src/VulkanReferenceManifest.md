...
|---|---|---|
| `src/GPU.h` | `src/GPU.h` | Core compatibility adaptation: the current upstream GPU owner keeps display-capture registers outside the older Sapphire GPU2D unit. R16 adds a line-0 capture snapshot (control, engine-A display control, and screen swap) without changing register read/write behavior. |
| `src/GPU.cpp` | `src/GPU.cpp` | Same classification as the header. Capture start/end and VRAM capture-block ownership use the immutable frame control; savestate format is unchanged and reconstructs the transient latch after load. |
| `src/GPU_Soft.cpp` | `src/GPU2D_Soft.cpp` | Core compatibility adaptation for the current monolithic software GPU owner: per-line capture mixing and destination writes consume the R16 line-0 snapshot. The DS RGB555 mixing and capture-range VRAM dirty behavior remain current-core code. |
