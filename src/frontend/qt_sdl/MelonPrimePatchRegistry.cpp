...
namespace MelonPrime {
    ...
    namespace {
        ...
        {
            InGameAspectRatio_ApplyOnce(ctx.emu, ctx.cfg, ctx.rom);
        }
        void Apply_OsdColor(const PatchCtx& ctx)
        {
            OsdColor_ApplyOnce(ctx.emu, ctx.cfg, ctx.rom);
        }
        ...
        {
            InstantAimFollow_ApplyOnce(ctx.nds, ctx.cfg, ctx.rom.romGroupIndex);
        }
        void Apply_ShowHeadshotOnline(const PatchCtx& ctx)
        {
            ShowHeadshotOnline_ApplyOnce(ctx.nds, ctx.cfg, ctx.rom.romGroupIndex);
        }
        void Apply_ShowEnemyHpMeterOnline(const PatchCtx& ctx)
        {
            ShowEnemyHpMeterOnline_ApplyOnce(ctx.nds, ctx.cfg, ctx.rom.romGroupIndex);
        }
        void Apply_DisableDoubleDamageMultiplier(const PatchCtx& ctx)
        {
            DisableDoubleDamageMultiplier_ApplyOnce(ctx.nds, ctx.cfg, ctx.rom.romGroupIndex);
        }
        void Apply_NoPickingUpSpecificItems(const PatchCtx& ctx)
        {
            NoPickingUpSpecificItems_ApplyOnce(ctx.nds, ctx.cfg, ctx.rom.romGroupIndex);
        }
        ...
        {
            ExpandStageMatrix_ApplyIfLoaded(ctx.nds, ctx.cfg, ctx.rom.romGroupIndex);
        }
        ...
        {
            ShowHeadshotOnline_RestoreOnce(ctx.nds, ctx.rom.romGroupIndex);
        }
        void Restore_ShowEnemyHpMeterOnline(const PatchCtx& ctx)
        {
            ShowEnemyHpMeterOnline_RestoreOnce(ctx.nds, ctx.rom.romGroupIndex);
        }
        void Restore_DisableDoubleDamageMultiplier(const PatchCtx& ctx)
        {
            DisableDoubleDamageMultiplier_RestoreOnce(ctx.nds, ctx.rom.romGroupIndex);
        }
        void Restore_NoPickingUpSpecificItems(const PatchCtx& ctx)
        {
            NoPickingUpSpecificItems_RestoreOnce(ctx.nds, ctx.rom.romGroupIndex);
        }
        ...
        //    singleton assumption holds; the registry does not change it.
        ...
        struct PatchEntry {
            ...
            void (*resetState)();                // per-module state reset
        };
        ...
        constexpr PatchEntry kPatchRegistry[] = {
            ...
            // isInGame branch (game-state-dependent); that call bypasses the
            // registry by design. GameJoin apply + game-leave restore live here.
            { "OsdColor",
              PatchSite_GameJoin, RF_OnLeave,
