...
| 項目 | 監査結果 |
|---|---|
| P-14 / `PrePollRawInput` | Round 5 では P-19 / P-20 に吸収された流れだが、**現コードでは関数・呼び出しともに残っている** |
| P-20 / PrePollRawInput 除去 | **文書上の到達点** としては成立するが、**現コードでは未完了** |
| P-22 / DeferredDrain 分離 | 分離自体は有効。ただし **配置説明は旧文書の「RunFrame 直後」ではなく、現コードでは `drawScreen()` 後** |
...
| P-14 | 歴史的段階 | ⚠ 文書上は吸収 / 現コード残置 | PrePoll バッチドレイン。Round 5 では P-19 / P-20 に吸収された流れだが、現コードでは `PrePollRawInput()` がまだ残っている | PrePoll バッチドレイン (Stuck Keys 修正), PrePollRawInput |
...
| P-20 | PrePollRawInput 除去 | ⚠ 文書上の到達点 / 現コード未完了 | Round 5 の意図としては P-19 により不要化。ただし現コードでは `PrePollRawInput()` 実装・呼び出しが残っている | PrePollRawInput 除去 |
...
| P-22 | DeferredDrain 分離 | ✅ + 注記 | PollAndSnapshot から drain を切り離し。旧文書では RunFrame 直後配置だが、現コードでは後続変更により `drawScreen()` 後 | DeferredDrainInput 呼び出し追加 |
...
**ただし現コード監査結果**
- `MelonPrimeCore::PrePollRawInput()` はまだ実装されている
- `EmuThread.cpp` からの呼び出しも残っている
- したがって、**「P-14 は完全に消えた」ではなく「歴史的には吸収予定だが、現コードではまだ残置」** と書くのが正確
...
**監査注記**
- 現コードでは `PrePollRawInput()` 実装も呼び出しもまだ残っている
- そのため本項目は、**「達成済みの現状」ではなく「Round 5 が目指した到達点」** として読む必要がある
...
### 現コード監査ベースの注記
- `PrePollRawInput()` はまだ残っているため、P-20 は未完了
- `DeferredDrainInput()` は後続変更により **`drawScreen()` 後** に置かれている
...
したがって、**文書上の設計到達点** と **現コードの実装状態** は分けて読む必要がある。
...
1. **P-14 は歴史的に重要だが、現コードではまだ残っている**  
   したがって「完全廃版」とは書かない。
2. **P-20 は Round 5 文書の到達点であり、現コードでは未完了**  
   したがって「適用済み」ではなく「設計上の到達点」として扱う。
3. **P-22 の本質は drain 分離であり、配置は後続変更で上書きされている**  
   現コードでの最終位置は `drawScreen()` 後。
...
| `MelonPrime.h` | OPT C/D/G/L/O/W、P-3、P-17、P-18 |
| `MelonPrime.cpp` | OPT D/E/K/L/O/W/Z1/Z2/Z3/Z4、FIX-3、P-14、P-20b、P-20c、P-22（現コード残置あり） |
| `MelonPrimeGameInput.cpp` | OPT F/O/Q/Z2/Z3、FIX-2、P-3、P-17、P-18 |
| `MelonPrimeInGame.cpp` | OPT A/B/G/H/J/Z2/Z5、R1 constexpr テーブル |
...
| `MelonPrimeRawInputState.cpp` | OPT S/T、R2 button precedence / fetch_add / LUT、P-1、P-6 |
| `MelonPrimeRawInputWinFilter.h` | R1 drain helper、R2 overload、P-22 |
| `MelonPrimeRawInputWinFilter.cpp` | FIX-1、OPT R/U、R1/R2、P-19、P-22、P-26b 旧コメント残骸 |
...
| `EmuThread.cpp` | P-11、P-12、P-13、P-14、P-15、P-16、P-22、P-24、P-25、P-26a、P-27、P-32 相当の後続配置変更 |
| `EmuInstance.h` | P-15 / P-21 |
| `EmuInstanceInput.cpp` | P-15、P-21、P-23 |
...
5. **5** で、入力遅延、フレーム順序、VSync 復帰、サブピクセル蓄積、stuck keys 根本修正、syscall 削減までを含む**パイプライン再設計案とその適用履歴**が整理された  
   ただし監査結果として、P-14 / P-20 / P-22 / P-26b には **文書上の到達点** と **現コード状態** の差分がある  

つまりこの 1～5 の統合結果は、単なる「速くするための断片集」ではなく、
