...
#include "MelonPrimeGameRomAddrTable.h" // �ǉ�
...
namespace MelonPrime {
    ...
    struct alignas(64) GameAddressesHot {
        ...
        // 64byte���E�ɍ��킹�邽�߂̃p�f�B���O (���v17��u32�����邽�߁A68bytes�B128bytes�܂Ŗ��߂�)
        melonDS::u32 _pad[15];
    };
    ...
    private:
        ...
        // ★ 改善3: std::function → メンバ関数ポインタディスパッチ
        //   FrameAdvanceOnce() の毎呼び出しで std::function::operator bool() と
        //   型消去された間接呼び出しを排除。直接メンバ関数ポインタ1回で完結。
        ...
        // ★ 改善1: 毎フレームの winId() WinAPI 呼び出しを排除
        ...
        // ★ 改善2: IsJoyDown/IsJoyPressed 宣言削除
        //   → UpdateInputState 内でラムダに直接インライン展開
        //   (別翻訳単位の非インライン関数呼び出しオーバーヘッドを排除)
        ...
        // ★ 改善3: FrameAdvanceOnce → ディスパッチ + 2つの実装
