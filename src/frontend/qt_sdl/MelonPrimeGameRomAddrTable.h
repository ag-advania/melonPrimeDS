...
#define MP_ROM_FIELDS_HUD(X) \
    ...
    X(ADDR, currentCameraSequence,    CurrentCameraSequence,     0x020DBB70u, 0x020DBB30u, 0x020D9CB0u, 0x020DA530u, 0x020DA550u, 0x020DA5D0u, 0x020D33A4u) /* u32 pointer to the active CameraSequence instance (0 = none); instance +0x00 flags bit 2 (0x04) = BlockInput, the same byte the native upper-HUD tests before returning ahead of the crosshair renderer */ \
    X(ADDR, scanVisorState,           ScanVisorState,            0x020E0568u, 0x020E0528u, 0x020DE690u, 0x020DEF10u, 0x020DEF30u, 0x020DEFB0u, 0x020D7D13u) /* u8 selected visor: 0 = Combat, 1 = Scan; the byte the visor switch writes and the native regular-HUD gate compares against 0. KR1_0 is a field of the HUD/player state block at 020D7D00 +0x13, read directly like the others */ \
