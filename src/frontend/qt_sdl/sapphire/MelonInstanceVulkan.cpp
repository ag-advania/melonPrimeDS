// MelonInstanceVulkan.cpp
//
// FUNCTION-DEFINITION-GRANULARITY port of `MelonDSAndroid::MelonInstance`'s Vulkan presentation
// pipeline from the reference Android frontend (melonDS-android tag 0.7.0.rc4,
// app/src/main/cpp/MelonInstance.cpp, ~8058 lines). Every KEPT function body below is byte-verbatim
// against that reference except where a `MELONPRIME-PC-ADAPT` comment marks a specific, minimal,
// desktop-forced edit. See MelonInstanceVulkan.h for the trimmed class declaration and the rationale
// for each adapted member/stub.
//
// ============================================================================================
// KEPT (reference MelonInstance.cpp line ranges -> this file):
//   37-70     file-scope constants (kRewindBufferSize, kVulkanFastForward*, kSoftPacked* flags, ...)
//   95-119    packedResolvedLineHasAnyUsefulPixel (fwd decl) + packedLineNeedsCompMode7Live3dFallback
//   121-309   hasMatchingLatchedSoftPackedSnapshot .. softPackedFrameNeedsReusablePreviousFrame
//             (the soft-packed classification helper family; all reachable from latchSoftPackedFrameSnapshot
//             and/or updateVulkanTemporal3dHistoryGate)
//   503-663   collectPackedScreenStatsFromSnapshot
//   682-769   packedResolvedLineHasAnyUsefulPixel (def) .. lineMaskHasAnyValidLine
//   880-1009  getConfiguredVulkanScale, getEffectiveVulkanRenderScale, getConfiguredVulkanBackendMode,
//             makeLegacyFrameQueuePolicy, makeVulkanRealtimeFrameQueuePolicy,
//             makeVulkanLateRealtimeFrameQueuePolicy, makeVulkanFastForwardFrameQueuePolicy,
//             constrainGraphicsHardwareFrameQueuePolicy, isPresentationDeadlineExpired,
//             makeFrameQueuePolicy, prepareRenderFrame
//   1011-1053 CopyCompositedFrameToScreenshot
//   1281-1371 MelonInstance::precompileVulkanPipelines
//   1421-1804 MelonInstance::runFrame -- SPLIT at the emu-step line (`u32 nLines = nds->RunFrame();`,
//             reference line 1590) into runFrameVulkanPreStep()/runFrameVulkanPostStep(nLines); see the
//             "Part 2: runFrame split" note below.
//   1806-1823 MelonInstance::handleVulkanRuntimeFailure
//   1948-1960 MelonInstance::waitForPresentationFrame
//   1962-1980 MelonInstance::attachVulkanSurface (ANativeWindow* param adapted to void*)
//   1982-1991 MelonInstance::resizeVulkanSurface
//   1993-2002 MelonInstance::configureVulkanSurface
//   2004-2010 MelonInstance::detachVulkanSurface
//   2012-2236 MelonInstance::presentVulkanFrame
//   2238-2263 MelonInstance::requestVulkanPresentationResync
//   2265-2274 MelonInstance::requestVulkanFastForwardPresentationTransition
//   3949-3973 MelonInstance::updateVulkanFastForwardRenderScale
//   4393-4451 MelonInstance::updateVulkanScreenshot
//   4507-4723 MelonInstance::logVulkanPerformanceIfNeeded
//   4737-4761 MelonInstance::clearLatchedSoftPackedFrameSnapshot
//   4763-4812 MelonInstance::updateVulkanTemporal3dHistoryGate
//   4814-4818 MelonInstance::isVulkanTemporal3dHistoryGateActive
//   4819-8056 MelonInstance::latchSoftPackedFrameSnapshot (the entire soft-packed latch pipeline, incl.
//             all its internal lambdas; this range has ZERO references to JNI/eventMessenger/
//             RetroAchievements/rumble/camera/net/screenshotRenderer/save-managers/rewindManager --
//             confirmed by grepping the extracted range before writing this file. It DOES reference
//             GPU2D::SoftRenderer / StructuredVulkan2D* symbols, which belong to the parallel agent's
//             in-progress GPU2D_Soft.* work; see probe results.)
//
// OMITTED (reference line ranges; not present in this file at all):
//   loadRom/loadGbaRom/loadRumblePak/loadGbaMemoryExpansion/loadGbaAnalogInput/loadGbaRumblePak/
//     bootFirmware/start/reset/stop/touchScreen/releaseScreen/pressKey/releaseKey/readAudioOutput/
//     setAudioOutputSkew/takeScreenshot/loadCheats/sendNetPacket/receiveNetPacket/getPresentationFrame
//     (desktop owns ROM/save/input/audio/netplay/cheats via its own EmuThread/EmuInstance code)
//   updateConfiguration, requestNdsSaveWrite/requestGbaSaveWrite/requestFirmwareSaveWrite,
//     areSaveStatesAllowed/saveState/loadState/getRewindWindow/loadRewindState (savestate cluster;
//     MelonInstanceVulkan::updateConfiguration is a NEW minimal replacement, see header)
//   setupAchievements/unloadRetroAchievementsData/getRichPresenceStatus/getRuntimeAchievements/
//     getRuntimeAchievementBuckets/getRuntimeSubsetIds (RetroAchievements JNI cluster)
//   2764-3948, 4725-4736: entire "*ForDebug" capture/dump cluster, dumpDebugSnapshot,
//     ensurePreparedVulkanDebugSnapshot/hasPreparedVulkanDebugSnapshot/prepareOpenGlDebugSnapshot/
//     clearPrepared*DebugSnapshot, dense-screen-burst-capture family, saveRewindState,
//     buildSoftPackedFrameMetaJson (verified unreferenced by any kept function -- only ever called from
//     the omitted debug/dense-burst cluster), collectPackedScreenStats (verified dead code in the
//     reference itself: zero call sites anywhere, superseded by collectPackedScreenStatsFromSnapshot),
//     packedRawLineIsZero (verified unreferenced), expandPackedColor6ToRgba8/encodePackedControlToRgba8/
//     encodeBinaryMaskToRgba8/expandPackedPixelsToRgbaVector/convertSoftPackedPlaneToRgbaVector/
//     encodeLineMaskToRgbaVector (verified debug-capture-only via call-site audit)
//   4189-4369: MelonInstance::updateRenderer -- DELETED (W2 lead ruling), not a straight port; see
//     the class-level comment on the empty updateRenderer() stub below for why (desktop renderer
//     lifecycle is owned by EmuThread::updateRenderer() instead). Its two call sites (in
//     precompileVulkanPipelines/runFrameVulkanPreStep) still call updateRenderer() -- kept
//     byte-verbatim in call STRUCTURE -- but the method body is now that no-op stub.
//
// STUBBED (private no-op member functions declared in MelonInstanceVulkan.h; kept call sites in
// runFrameVulkanPostStep/precompileVulkanPipelines/updateRenderer/requestVulkanPresentationResync stay
// byte-verbatim against these no-ops):
//   clearPreparedVulkanDebugSnapshot, clearPreparedOpenGlDebugSnapshot, prepareOpenGlDebugSnapshot,
//   maybeCaptureDenseScreenBurstFrame, saveRewindState
//
// Part 2 -- runFrame split rationale: the reference runFrame() has ONE early return
// (`if (!nds->IsRunning()) return 0;`, before any Vulkan-pipeline work) and 8 locals that are computed
// before the emu-step line and read after it: measuringVulkan, runFrameStartNs, ndsRunStartNs,
// vulkanRenderScale, frameBackend, frameQueuePolicy, renderFrame, isRendererAccelerated. These are
// promoted to `crossStep_*` members (MelonInstanceVulkan.h) written in runFrameVulkanPreStep() and read
// in runFrameVulkanPostStep(); ndsRunEndNs, despite being declared before the emu-step line in the
// reference, is entirely written+read AFTER it, so it stays a plain post-step local instead of being
// promoted. runFrameVulkanPreStep() returns bool (true = proceed) in place of the reference's
// `u32 runFrame()` return, so its early "not running" exit can signal the caller (desktop EmuThread) to
// skip both its own nds->RunFrame() call and runFrameVulkanPostStep() for that tick, matching the
// reference's early-return semantics exactly. A whole-function (unsplit) fallback was considered per the
// task brief but rejected: the split is mechanically clean (single early-return, well-defined crossing
// set, no other control-flow hazards), and it lets EmuThread keep its own generic nds->RunFrame() call
// site shared across all renderers instead of special-casing Vulkan's emu step.
// ============================================================================================
...
// MELONPRIME-PC-ADAPT: reference declares these two in MelonDS.h (the Android app-layer orchestrator,
// out of scope to port) and defines them in MelonDS.cpp; desktop definitions live in SapphireSupport.cpp
// (src/frontend/qt_sdl/sapphire/SapphireSupport.cpp). Forward-declared here so the kept function bodies
// below (precompileVulkanPipelines/handleVulkanRuntimeFailure) that reference them stay byte-verbatim.
// getVulkanDiagnosticFlags() is currently unreferenced in this file: its only kept call site was
// inside the VulkanRuntime[Renderer] log line in the now-deleted updateRenderer() (see W2 note
// above); left declared (harmless, TU-local) in case a future desktop renderer-lifecycle owner
// wants the same diagnostic log.
extern std::shared_ptr<MelonEventMessenger> eventMessenger;
extern melonDS::u32 getVulkanDiagnosticFlags();

// MELONPRIME-PC-ADAPT: forward-declared here rather than pulled in via a header, matching the same
// pattern the verbatim-ported renderer/VulkanOutput.cpp and renderer/VulkanSurfacePresenter.cpp already
// use for these same MelonDSAndroid free functions (defined for desktop in SapphireSupport.cpp).
...
// ---- soft-packed helper functions (reference 95-119, 121-309, 503-663, 682-769) ----
...
// ---- vulkan scale/backend-mode + frame-queue-policy helpers (reference 880-1009) ----
...
// ---- screenshot compositing helper (reference 1011-1053) ----
...
// MELONPRIME-PC-ADAPT: new constructor/destructor (reference MelonInstance.cpp:1056-1119 is not in the
// kept-function list; it wires JNI-only construction args that are out of scope for the Vulkan pipeline
// port -- see MelonInstanceVulkan.h). Part 3 desktop wiring constructs one of these per Vulkan-renderer
// EmuInstance, passing the already-constructed NDS* and the shared EmulatorConfiguration.
MelonInstance::MelonInstance(int instanceId, melonDS::NDS* nds, std::shared_ptr<EmulatorConfiguration> configuration)
    : instanceId(instanceId)
    , nds(nds)
    , currentConfiguration(std::move(configuration))
    , screenshotRenderer(std::make_unique<ScreenshotRenderer>(nullptr))
    // Rewind capture starts disabled (desktop savestate/rewind wiring is out of scope here; see
    // saveRewindState's no-op stub). A capturing interval of 1 (rather than 0) avoids a divide-by-zero
    // in RewindManager::RewindWindowSize() if rewind is ever toggled on without also setting a real
    // interval.
    , rewindManager(false, 0, 1, 0, 0)
    , retroAchievementsManager(std::make_unique<RetroAchievementsManagerStub>())
{
}
...
// Reference MelonInstance.cpp:1421-1589 (verbatim through the emu-step line; see split note above).
// MELONPRIME-PC-ADAPT: split from reference runFrame(); returns false (was: an early "return 0" ) when
// nds->IsRunning() is false, signalling the caller (EmuThread) to skip both the emu step and
// runFrameVulkanPostStep() for this tick, matching the reference's early-return semantics.
bool MelonInstance::runFrameVulkanPreStep()
{
    // MELONPRIME-PC-ADAPT: crossStep_* members below replace locals that cross the reference
    // runFrame()'s Pre/Post split boundary (this file's own emu-step line). Each is written here
    // and consumed in runFrameVulkanPostStep(); see MelonInstanceVulkan.h for the full list.
    crossStep_measuringVulkan = currentConfiguration->renderer == Renderer::Vulkan;
    crossStep_runFrameStartNs = crossStep_measuringVulkan ? PerfNowNs() : 0;
    crossStep_ndsRunStartNs = 0;
    ...
    if (!nds->IsRunning())
        return false; // MELONPRIME-PC-ADAPT: was "return 0;" (u32 runFrame() early exit)

    // MELONPRIME-PC-ADAPT: reference forwards slot2AnalogX/Y (GBA slot-2 accessory analog input,
    // e.g. a rumble/tilt pak) into GBACart::SetInput(Input_AnalogX/Y, float). Our fork's
    // GBACart::SetInput(int num, bool pressed) (src/GBACart.h) is a digital-only button model with
    // no analog-input enum or plumbing -- desktop input is entirely MelonPrime-owned (see
    // src/frontend/qt_sdl/MelonPrimePlatformInput.h) and slot2AnalogX/Y are never written from any
    // desktop UI/config surface for this accessory. Neutralized rather than ported.
    ...
    int screenWidth;
    int screenHeight;
    crossStep_vulkanRenderScale = 1;
    if (currentRenderer == Renderer::OpenGl)
    {
        // MELONPRIME-PC-ADAPT: reference's `GLRenderer` names its OpenGL Renderer3D subclass
        // directly (Sapphire's old-generation architecture -- GetRenderer3D() returns Renderer3D&
        // there too). Our fork's OpenGL Renderer3D subclass is named `GLRenderer3D` (GPU3D_OpenGL.h);
        // `GLRenderer` in this fork instead names the OUTER 2D+3D-driving Renderer subclass
        // (GPU_OpenGL.h, an unrelated hierarchy -- not castable from Renderer3D&). This branch is
        // structurally dead in practice for this MelonInstance (currentRenderer stays
        // Renderer::Vulkan for its whole lifetime -- see W2 note on updateRenderer()) but must still
        // type-check.
        int scale = static_cast<GLRenderer3D &>(nds->GPU.GetRenderer3D()).GetScaleFactor();
    ...
    {
        auto& renderer3D = static_cast<VulkanRenderer3D&>(nds->GPU.GetRenderer3D());
        crossStep_vulkanRenderScale = std::max(renderer3D.GetScaleFactor(), 1);
        screenWidth = 256 * crossStep_vulkanRenderScale;
        screenHeight = (192 + 1) * crossStep_vulkanRenderScale;
    }
    else
    {
        screenWidth = 256;
    ...
    crossStep_frameBackend = (currentRenderer == Renderer::Vulkan) ? FrameBackend::VulkanImage : FrameBackend::OpenGlTexture;
    crossStep_frameQueuePolicy = makeFrameQueuePolicy(currentRenderer, crossStep_vulkanRenderScale);
    if (currentRenderer == Renderer::Vulkan)
    {
        auto& renderer3D = static_cast<VulkanRenderer3D&>(nds->GPU.GetRenderer3D());
        const bool needsReusablePreviousFrame = updateVulkanTemporal3dHistoryGate();
        crossStep_frameQueuePolicy = constrainGraphicsHardwareFrameQueuePolicy(
            crossStep_frameQueuePolicy,
            renderer3D.GetActiveBackendMode() == VulkanRenderer3D::BackendMode::GraphicsHardware,
            needsReusablePreviousFrame);
    }
    crossStep_renderFrame = nullptr;
    ...
    {
        Frame* candidateFrame = frameQueue.getRenderFrame(crossStep_frameQueuePolicy);
        ...
        if (readyForReuse)
        {
            crossStep_renderFrame = candidateFrame;
            break;
        }

        frameQueue.recycleRenderFrame(candidateFrame);
    }
    if (crossStep_renderFrame == nullptr && currentRenderer == Renderer::Vulkan && vulkanOutput != nullptr)
    {
    ...
    prepareRenderFrame(crossStep_renderFrame);
    if (crossStep_renderFrame != nullptr)
        frameQueue.validateRenderFrame(crossStep_renderFrame, screenWidth, screenHeight * 2, crossStep_frameBackend);

    if (currentRenderer == Renderer::Vulkan)
    {
        if (crossStep_renderFrame != nullptr)
            crossStep_renderFrame->renderTimelineValue = 0;

        if (crossStep_renderFrame != nullptr && vulkanOutput != nullptr)
        {
            auto& renderer3D = static_cast<VulkanRenderer3D&>(nds->GPU.GetRenderer3D());
            if (vulkanOutput->ensureFrameResources(crossStep_renderFrame, screenWidth, screenHeight * 2))
            {
    ...
    crossStep_isRendererAccelerated = nds->GPU.GetRenderer3D().Accelerated;
    if (crossStep_isRendererAccelerated && crossStep_frameBackend == FrameBackend::OpenGlTexture && crossStep_renderFrame != nullptr)
    {
        // MELONPRIME-PC-ADAPT: GPU.FrontBuffer -> GPU.GetFrontBufferIndex() (see GPU.h class-level
        // comment on GetFrontBufferIndex()).
        int backBuffer = nds->GPU.GetFrontBufferIndex() ? 0 : 1;
        nds->GPU.GetRenderer3D().SetOutputTexture(backBuffer, crossStep_renderFrame->frameTexture);
    }

    if (crossStep_measuringVulkan)
    {
        crossStep_ndsRunStartNs = PerfNowNs();
        vulkanSetupCpuWindow.Add(crossStep_ndsRunStartNs - crossStep_runFrameStartNs);
    }
...
// Reference MelonInstance.cpp:1591-1804 (verbatim after the emu-step line; see split note above).
void MelonInstance::runFrameVulkanPostStep(melonDS::u32 nLines)
{
    // MELONPRIME-PC-ADAPT: ndsRunEndNs does not cross the split boundary in the reference (it is
    // written and read entirely after the emu-step line), so it stays a plain local here instead of
    // a crossStep_ member.
    u64 ndsRunEndNs = 0;
    if (crossStep_measuringVulkan)
    {
        ndsRunEndNs = PerfNowNs();
        vulkanNdsRunCpuWindow.Add(ndsRunEndNs - crossStep_ndsRunStartNs);
    }
    ...
    if (currentRenderer == Renderer::Vulkan && crossStep_renderFrame != nullptr)
    {
        ...
        if (vulkanRegularCaptureTransitionResyncPending)
        {
            ...
            if (nds != nullptr)
            {
                // MELONPRIME-PC-ADAPT: reference dynamic_casts GPU::GetRenderer2D()'s returned
                // base-class GPU2D::Renderer2D& down to the concrete GPU2D::SoftRenderer (to check
                // "is the active 2D renderer the software one" -- it can also be a hardware-backed
                // Renderer2D there). Our GPU::GetRenderer2D() (see GPU.h/GPU2D_Soft.h) already
                // returns the concrete StructuredVulkan2DRendererBridge type directly (there is no
                // alternative Renderer2D-family type it could be), so no cast/nullptr-check is
                // needed here; kept as a pointer-taking `if` to preserve the reference's call-site
                // shape.
                if (auto* renderer2D = &nds->GPU.GetRenderer2D())
    ...
    if (currentRenderer == Renderer::Vulkan)
    {
        if (vulkanOutput)
        {
            ...
            if (shouldHoldPreviousFrame)
            {
                ...
                if (areRendererDebugBgObjLogsEnabled())
                {
                    Platform::Log(
                        ...
                        crossStep_renderFrame != nullptr ? static_cast<unsigned>(crossStep_renderFrame->frameId) : 0u,
                        frontbuf,
            ...
            else if (crossStep_renderFrame != nullptr && hasPresentableSoftPackedFrame && !isFrameUploaded)
            {
    ...
    else if (!crossStep_isRendererAccelerated)
    {
        // MELONPRIME-PC-ADAPT: reference reads its GPU-level double-buffered `Framebuffer[frontbuf][0/1]`
        // raw pointers directly. Our fork's equivalent flat CPU BGRA buffers are owned by the active
        // Renderer (SoftRenderer::Framebuffer[][], see GPU_Soft.h) and exposed generically via
        // Renderer::GetFramebuffers(void** top, void** bottom) (GPU.h) -- already front-buffer-aware
        // (SoftRenderer::GetFramebuffers() internally computes `BackBuffer ^ 1`, matching
        // GetFrontBufferIndex() above), so `frontbuf` isn't needed for this call.
        void* topFb = nullptr;
        void* bottomFb = nullptr;
        if (nds->GPU.GetFramebuffers(&topFb, &bottomFb) && topFb != nullptr && bottomFb != nullptr)
        {
            glBindTexture(GL_TEXTURE_2D, crossStep_renderFrame->frameTexture);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 256, 192, GL_RGBA, GL_UNSIGNED_BYTE, topFb);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 192 + 2, 256, 192, GL_RGBA, GL_UNSIGNED_BYTE, bottomFb);
    ...
    if (currentRenderer == Renderer::Vulkan)
    {
        if (hasValidFrame)
        {
            lastCompletedVulkanFrame = crossStep_renderFrame;
            lastCompletedVulkanScale = std::max(crossStep_vulkanRenderScale, 1);
        }
    }
    else
    {
        lastCompletedVulkanFrame = nullptr;
        lastCompletedVulkanScale = 1;
        screenshotRenderer->renderScreenshot(&nds->GPU, currentRenderer, crossStep_renderFrame);
    }
    ...
    if (hasValidFrame)
    {
        maybeCaptureDenseScreenBurstFrame(
            currentRenderer == Renderer::Vulkan ? crossStep_renderFrame : nullptr,
    ...
    if (currentRenderer == Renderer::Vulkan && shouldCaptureRewindState)
        (void)updateVulkanScreenshot(hasValidFrame ? crossStep_renderFrame : lastCompletedVulkanFrame, hasValidFrame ? std::max(crossStep_vulkanRenderScale, 1) : lastCompletedVulkanScale, true);
    ...
    {
        EGLDisplay currentDisplay = eglGetCurrentDisplay();
        if (crossStep_frameBackend == FrameBackend::OpenGlTexture)
        {
            crossStep_renderFrame->renderFence = eglCreateSyncKHR(currentDisplay, EGL_SYNC_FENCE_KHR, nullptr);
            glFlush();
        }
        else
        {
            crossStep_renderFrame->renderFence = 0;
        }
        frameQueue.pushRenderedFrame(crossStep_renderFrame, crossStep_frameQueuePolicy);
    }
    else if (crossStep_renderFrame != nullptr)
    {
        frameQueue.discardRenderedFrame(crossStep_renderFrame);
    }
    ...
    if (currentRenderer == Renderer::Vulkan)
    {
        ...
        vulkanRunFrameCpuWindow.Add(runFrameEndNs - crossStep_runFrameStartNs);
        logVulkanPerformanceIfNeeded();
    }

    return; // MELONPRIME-PC-ADAPT: was "return nLines;" -- caller (EmuThread) already has nLines from its own nds->RunFrame() call
}
...
// Reference MelonInstance.cpp:1948-1960 (kept verbatim).
...
// Reference MelonInstance.cpp:1962-1980 (kept; parameter type and one call site adapted, tagged below).
int MelonInstance::attachVulkanSurface(void* window, u32 width, u32 height) // MELONPRIME-PC-ADAPT: ANativeWindow* -> void* (HWND)
{
    if (window == nullptr)
        return 0;
    ...
    {
        // MELONPRIME-PC-ADAPT: reference calls ANativeWindow_release(window) here (Android transfers
        // native-window ownership to the callee). Desktop passes a borrowed HWND (see
        // VulkanSurfacePresenter::attachSurface's own "presenter does not own the window" adaptation
        // note); there is nothing to release.
        return 0;
    }
...
// Reference MelonInstance.cpp:1982-1991 (kept verbatim).
...
// Reference MelonInstance.cpp:1993-2002 (kept verbatim).
...
// Reference MelonInstance.cpp:2004-2010 (kept verbatim).
...
// Reference MelonInstance.cpp:2012-2236 (kept verbatim).
...
// Reference MelonInstance.cpp:2238-2263 (kept verbatim).
...
// Reference MelonInstance.cpp:2265-2274 (kept verbatim).
...
// MELONPRIME-PORT: W2 (lead ruling) -- reference MelonInstance.cpp:4189-4369's updateRenderer()
// is DELETED, not ported. On desktop, renderer lifecycle (construction/destruction/swap of the
// active Renderer3D backend across Software/OpenGL/Vulkan/Compute) belongs entirely to
// EmuThread::updateRenderer() (src/frontend/qt_sdl/EmuThread.cpp:1298), which already constructs
// this very MelonInstance (and its VulkanGpuRenderer, via nds->GPU.SetRenderer()) when switching
// TO the Vulkan renderer -- see EmuThread.cpp:1328 case renderer3D_Vulkan. Porting reference's
// updateRenderer() here would create a second, competing renderer-swap authority operating on the
// same GPU: it called `nds->GPU.SetRenderer3D(...)` (no equivalent on our GPU -- see GPU.h; our
// GPU only exposes the coarser `SetRenderer(std::unique_ptr<Renderer>&&)`, which replaces the
// WHOLE 2D+3D Renderer, not just Rend3D) and referenced reference-only renderer type names
// (`GLRenderer`/`ComputeRenderer` construction, `SoftRenderer::SetThreaded`) that don't match this
// fork's actual GL/Compute renderer classes or constructors. Both call sites below (still call
// `updateRenderer()`, matching reference's structure) are therefore routed to a tagged no-op: this
// MelonInstance only ever exists while currentRenderer == Renderer::Vulkan (constructed by
// EmuThread specifically for that case, see above), so there is nothing for this stub to do beyond
// clearing the dirty flag the caller already clears right after calling it.
void MelonInstance::updateRenderer()
{
    // MELONPRIME-PC-ADAPT: reference MelonInstance::updateRenderer() (4189-4369) did TWO things:
    // (a) create/init the MelonInstance-owned VulkanOutput compositor, and (b) construct and install
    // the nds Renderer3D. W2's lead ruling correctly moved (b) to EmuThread::updateRenderer()
    // (which builds VulkanGpuRenderer), but (a) is MelonInstance's own state that nothing else
    // creates -- dropping it left vulkanOutput null, so runFrameVulkanPostStep hit
    // handleVulkanRuntimeFailure("missing VulkanOutput"). This restores ONLY block (a), verbatim
    // from reference lines 4200-4212; the renderer-swap half stays owned by EmuThread. This
    // MelonInstance is constructed only for Renderer::Vulkan (currentRenderer defaults to Vulkan),
    // so no newRenderer!=currentRenderer gating is needed.
    if (!vulkanOutput)
...
void MelonInstance::logVulkanPerformanceIfNeeded()
{
    ...
    // MELONPRIME-PC-ADAPT: GPU2D::SoftRenderer -> StructuredVulkan2DRendererBridge (see class-level
    // comment in GPU2D_Soft.h); no cast needed, GetRenderer2D() already returns the concrete type.
    StructuredVulkan2DRendererBridge::DebugCaptureStats captureStats{};
    if (nds != nullptr)
    {
        if (const auto* renderer2D = &nds->GPU.GetRenderer2D())
...
// Reference MelonInstance.cpp:4819-8056 (kept verbatim; entire soft-packed latch pipeline).
    ...
    bool useStructuredVulkan2D)
{
    ...
    // MELONPRIME-PC-ADAPT: GPU.Framebuffer[frontBuffer][0/1] -> GPU.GetAccelFramebuffer(frontBuffer, 0/1)
    // (see GPU.h/GPU_Soft.h class-level comments -- accelerated-stride channel, physical-screen-indexed).
    const u32* topPackedRaw = nds->GPU.GetAccelFramebuffer(frontBuffer, 0);
    const u32* bottomPackedRaw = nds->GPU.GetAccelFramebuffer(frontBuffer, 1);
    ...
    const auto* renderer2D = useStructuredVulkan2D
        ...
        : nullptr;
    const StructuredVulkan2DRendererBridge::DebugCaptureStats captureStats =
        renderer2D != nullptr ? renderer2D->GetDebugCaptureStats() : StructuredVulkan2DRendererBridge::DebugCaptureStats{};
    const u32* structuredTopPlane0 = renderer2D != nullptr ? renderer2D->GetStructuredVulkan2DPlane(frontBuffer, true, 0) : nullptr;
    const u32* structuredTopPlane1 = renderer2D != nullptr ? renderer2D->GetStructuredVulkan2DPlane(frontBuffer, true, 1) : nullptr;
    const u32* structuredTopControl = renderer2D != nullptr ? renderer2D->GetStructuredVulkan2DPlane(frontBuffer, true, 2) : nullptr;
    const u32* structuredBottomPlane0 = renderer2D != nullptr ? renderer2D->GetStructuredVulkan2DPlane(frontBuffer, false, 0) : nullptr;
    const u32* structuredBottomPlane1 = renderer2D != nullptr ? renderer2D->GetStructuredVulkan2DPlane(frontBuffer, false, 1) : nullptr;
    const u32* structuredBottomControl = renderer2D != nullptr ? renderer2D->GetStructuredVulkan2DPlane(frontBuffer, false, 2) : nullptr;
