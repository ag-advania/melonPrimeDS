// MELONPRIME-PC-ADAPT: enable Win32 Vulkan surface extension + Win32 window APIs (must precede vulkan.h)
...
// MELONPRIME-PC-ADAPT: ANativeWindow* -> void* (HWND)
int VulkanSurfacePresenter::attachSurface(void* window, u32 width, u32 height)
{
    ...
    // MELONPRIME-PC-ADAPT: VkWin32SurfaceCreateInfoKHR replaces VkAndroidSurfaceCreateInfoKHR
    VkWin32SurfaceCreateInfoKHR surfaceCreateInfo{};
    surfaceCreateInfo.sType = VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR;
    surfaceCreateInfo.hinstance = GetModuleHandleW(nullptr);
    surfaceCreateInfo.hwnd = static_cast<HWND>(window);

    // MELONPRIME-PC-ADAPT: vkCreateWin32SurfaceKHR replaces vkCreateAndroidSurfaceKHR
    if (vkCreateWin32SurfaceKHR(instance, &surfaceCreateInfo, nullptr, &surfaceState.surface) != VK_SUCCESS)
    {
        // MELONPRIME-PC-ADAPT: HWND not owned by presenter; no ANativeWindow_release equivalent
        return 0;
    }
...
void VulkanSurfacePresenter::destroySurfaceStateResources(SurfaceState& surfaceState)
{
    ...
    // MELONPRIME-PC-ADAPT: HWND not owned by presenter; no ANativeWindow_release equivalent (no-op)
}
...
bool VulkanSurfacePresenter::ensureSwapchain(SurfaceState& surfaceState)
{
    ...
    // MELONPRIME-PC-ADAPT: GetClientRect(HWND) replaces ANativeWindow_getWidth/getHeight
    RECT clientRect{};
    GetClientRect(static_cast<HWND>(surfaceState.window), &clientRect);
    u32 width = surfaceState.requestedWidth > 0 ? surfaceState.requestedWidth : static_cast<u32>(std::max<LONG>(1, clientRect.right - clientRect.left));
    u32 height = surfaceState.requestedHeight > 0 ? surfaceState.requestedHeight : static_cast<u32>(std::max<LONG>(1, clientRect.bottom - clientRect.top));
