...
namespace MelonPrime {
    ...
    void RawWorker::threadLoop() noexcept {
        ...
        while (m_runThread.load(std::memory_order_relaxed)) {
            ...
            // 2. メッセージキュー掃除
            //    PM_NOREMOVE で覗き見 → WM_INPUT なら先にバッファ吸い出し → PM_REMOVE で消す
            //    これにより GetRawInputBuffer からデータが消える問題を回避
            while (true) {
                BOOL hasMsg = FALSE;
                if (ntPeekMessage) {
