...
FrameQueuePolicy SapphireFrameQueueCore::sanitizePolicy(FrameQueuePolicy policy)
{
    policy.MaxBacklogDepth = std::max<u64>(1u, std::min<u64>(policy.MaxBacklogDepth, FRAME_QUEUE_SIZE - 1));
    return policy;
}

Frame* SapphireFrameQueueCore::getRenderFrame(const FrameQueuePolicy& requestedPolicy)
{
...
Frame* SapphireFrameQueueCore::getPresentFrame(
...
Frame* SapphireFrameQueueCore::getPresentCandidate(
    const FrameQueuePolicy& requestedPolicy,
    std::optional<std::chrono::time_point<std::chrono::steady_clock>> deadline)
{
    std::unique_lock lock(frameLock);
...
Frame* SapphireFrameQueueCore::getReusablePreviousFrame(const FrameQueuePolicy& requestedPolicy)
{
    std::unique_lock lock(frameLock);
    const FrameQueuePolicy policy = sanitizePolicy(requestedPolicy);
...
void SapphireFrameQueueCore::recycleRenderFrame(Frame* frame)
{
    std::unique_lock lock(frameLock);
    if (frame == nullptr)
        return;
...
void SapphireFrameQueueCore::commitPresentedFrame(Frame* frame, const FrameQueuePolicy& requestedPolicy)
{
    std::unique_lock lock(frameLock);
    if (frame == nullptr)
        return;
...
void SapphireFrameQueueCore::deferPresentedFrame(Frame* frame, const FrameQueuePolicy& requestedPolicy)
{
    std::unique_lock lock(frameLock);
...
void SapphireFrameQueueCore::pushRenderedFrame(Frame* frame, const FrameQueuePolicy& requestedPolicy)
{
    std::unique_lock lock(frameLock);
    const FrameQueuePolicy policy = sanitizePolicy(requestedPolicy);
...
void SapphireFrameQueueCore::discardRenderedFrame(Frame* frame)
{
    std::unique_lock lock(frameLock);
    frame->queuedAtNs = 0;
    freeQueue.push(frame);
...
void SapphireFrameQueueCore::requestPresentationResync()
{
    std::unique_lock lock(frameLock);
...
void SapphireFrameQueueCore::requestFastForwardPresentationTransition()
{
    std::unique_lock lock(frameLock);
...
FrameQueueStats SapphireFrameQueueCore::takeStatsSnapshotAndReset()
{
    std::unique_lock lock(frameLock);
...
void SapphireFrameQueueCore::rebuildFreeQueueLocked()
{
...
void SapphireFrameQueueCore::dropPendingFramesToBacklogLocked(u64 maxBacklogDepth, bool treatAsFastForwardSkip)
{
    const u64 nowNs = MelonDSAndroid::PerfNowNs();
...
void SapphireFrameQueueCore::recordPresentedFrameAgeLocked(Frame* frame, u64 nowNs)
{
...
void SapphireFrameQueueCore::recordDroppedFrameLocked(Frame* frame, PresentDropCause cause, u64 nowNs)
{
    if (frame == nullptr)
        return;
