...
void SapphireFrameLatchCore::clearLatchedSoftPackedFrameSnapshot()
{
...
bool SapphireFrameLatchCore::updateVulkanTemporal3dHistoryGate()
{
...
bool SapphireFrameLatchCore::isVulkanTemporal3dHistoryGateActive() const
{
    return vulkanTemporal3dHistoryGateFrames > 0;
}

bool SapphireFrameLatchCore::latchSoftPackedFrameSnapshot(
    const Frame* frame,
    int frontBuffer,
    bool screenSwap,
    bool useStructuredVulkan2D)
{
    ...
    const u32* topPackedRaw = nds_->GPU.Framebuffer[frontBuffer][0].get() != nullptr
        ? nds_->GPU.Framebuffer[frontBuffer][0].get()
        : nullptr;
    const u32* bottomPackedRaw = nds_->GPU.Framebuffer[frontBuffer][1].get() != nullptr
