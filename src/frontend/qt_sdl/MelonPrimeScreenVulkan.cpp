    ...
    const VulkanDirectFrameSnapshot& snapshot)
{
    ...
    const QSize requiredOverlay(
        std::max(1, snapshot.HudOverlay.width()),
        std::max(1, snapshot.HudOverlay.height()));
    ...
    {
        overlayCopySize = snapshot.HudOverlay.size();
        CopyImageTightly(
            staging + m_directOverlayOffset,
            snapshot.HudOverlay);
    }
        ...
        !snapshot.HudDestination.isEmpty())
    {
        ...
        drawQuad(
            ...
            static_cast<float>(
                snapshot.HudOverlay.width()) /
                m_directOverlaySize.width(),
            static_cast<float>(
                snapshot.HudOverlay.height()) /
                m_directOverlaySize.height(),
            1,
            0,
            {});
    ...
    VulkanDirectFrameSnapshot& snapshot)
{
    ...
    else
    {
        ...
        std::memcpy(
            snapshot.Screens[0].bits(),
            output.Top,
            nativeBytes);
        std::memcpy(
            snapshot.Screens[1].bits(),
            output.Bottom,
            nativeBytes);
    }
