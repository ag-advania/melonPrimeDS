...
enum class MenuLangId : int
{
    ...
    // Appended after the original 26-language set. New entries must always be
    // appended here (never inserted earlier in the enum): persisted config
    // values are the raw int (MenuLangId::First + ordinal), so inserting a
    // language in the middle would silently reassign every later language's
    // saved selection to a different language.
    Afrikaans,
    Irish,
    Icelandic,
    Azerbaijani,
