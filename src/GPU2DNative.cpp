...
{
    if (!StageDiagnosticsEnabled() || !structured)
        return;

    u64 displayPixels = 0u;
    u64 persistentSidecarUsed = 0u;
    u64 noRetainedCapture = 0u;
    u64 identityMismatch = 0u;
    u64 captureInactiveButCommittedAvailable = 0u;
    u64 currentCaptureDifferentBankButCommittedAvailable = 0u;
    u64 representativeMismatch = 0u;
    u64 pendingFeedbackOldVersion = 0u;
    u64 pendingFeedbackNewVersion = 0u;

    for (u32 screen = 0u; screen < 2u; ++screen)
    {
        const u32 referenceBase = screen * 4u * ScreenPixelCount
            + 3u * ScreenPixelCount;
        for (u32 line = 0u; line < ScreenHeight; ++line)
        {
            const u32 engine = input.ScreenSource[
                screen * ScreenHeight + line] & 1u;
            if (engine != 0u)
                continue;

            const LineState& state = input.Lines[line];
            if (((state.DispCnt >> 16u) & 3u) != 2u)
                continue;

            const u32 displayBank = (state.DispCnt >> 18u) & 3u;
            if ((state.LCDVRAMMap & (1u << displayBank)) == 0u)
                continue;

            const bool captureActive = state.CaptureEnable != 0u;
            const u32 destinationBank = (state.CaptureCnt >> 16u) & 3u;
            const u32 captureStart =
                (state.SpriteLatchValid >> CaptureStartLineShift)
                & CaptureStartLineMask;
            for (u32 x = 0u; x < ScreenWidth; ++x)
            {
                ++displayPixels;
                const u32 address = WrapLCDCHalfword(line * ScreenWidth + x);
                const u32 segment = displayBank * HighResCaptureSegmentsPerBank
                    + address / HighResCaptureSegmentHalfwords;
                const HighResCaptureProvenanceState& sidecar =
                    provenance[segment];
                const bool committedValid =
                    IsHighResCaptureCommittedIdentityValid(sidecar);
                const bool pendingValid =
                    (sidecar.ValidAndVersion & HighResCapturePendingWriteBit) != 0u
                    && sidecar.PendingIdentity.Valid
                    && sidecar.PendingIdentity.CompletionValue != 0u;
                const u32 committedVersion =
                    (sidecar.ValidAndVersion & HighResCaptureVersionBit) != 0u
                    ? 1u : 0u;
                const HighResCaptureReferenceDecision expected =
                    ResolveHighResCaptureReference(
                        committedValid, pendingValid, committedVersion,
                        line, displayBank, address, state.CaptureCnt,
                        captureStart);
                const u32 reference = structured[
                    referenceBase + line * ScreenWidth + x];

                if (!captureActive && committedValid)
                    ++captureInactiveButCommittedAvailable;
                if (captureActive && destinationBank != displayBank
                    && committedValid)
                {
                    ++currentCaptureDifferentBankButCommittedAvailable;
                }

                if ((reference & 0x80000000u) != 0u)
                {
                    ++persistentSidecarUsed;
                    if (expected.Version
                        == HighResCaptureReferenceVersion::Pending)
                    {
                        ++pendingFeedbackNewVersion;
                    }
                    else if (pendingValid && destinationBank == displayBank)
                    {
                        ++pendingFeedbackOldVersion;
                    }
                    continue;
                }
                ...
                {
                    // The shared physical resolver admitted a version but the
                    // structured wrapper rejected it. The only remaining
                    // fail-closed gate is the representative compact sample.
                    ++representativeMismatch;
                }
                else if ((sidecar.ValidAndVersion & HighResCaptureValidBit) != 0u
                    || (sidecar.ValidAndVersion
    ...
    Platform::Log(
        Platform::LogLevel::Info,
        "[GPU2DVRAMDisplaySidecar] backend=%s emulated=%llu scale=%u "
        "pixels=%llu persistentUsed=%llu noRetained=%llu "
        "identityMismatch=%llu representativeMismatch=%llu "
        "captureInactiveCommitted=%llu oppositeBankCommitted=%llu "
        "pendingOld=%llu pendingNew=%llu\n",
        backend,
        static_cast<unsigned long long>(emulatedFrame), scaleFactor,
        static_cast<unsigned long long>(displayPixels),
        static_cast<unsigned long long>(persistentSidecarUsed),
        static_cast<unsigned long long>(noRetainedCapture),
        static_cast<unsigned long long>(identityMismatch),
        static_cast<unsigned long long>(representativeMismatch),
        static_cast<unsigned long long>(
            captureInactiveButCommittedAvailable),
        static_cast<unsigned long long>(
            currentCaptureDifferentBankButCommittedAvailable),
        static_cast<unsigned long long>(pendingFeedbackOldVersion),
        static_cast<unsigned long long>(pendingFeedbackNewVersion));
}
