...
Custom HUD Perf（[melonprime-refactoring.md](../../../architecture/history/melonprime-refactoring.md) が正典）で徹底最適化済みで、
...
毎フレーム経路の全体像（プラットフォーム別）は [melonprime-aim-input.md](../../../architecture/input/aim-input.md) と
[melonprime-gameplay-runtime.md](../../../architecture/gameplay/runtime.md) §1/§9 が正典。
...
4. **キャッシュ/invalidation 規律の台帳化** — [melonprime-performance.md](../../../architecture/performance.md)
...
### V5-W7. per-frame パッチ/状態再評価（[別ロードマップ](melonprime-highres-fonts-v3-refactor-roadmap.md)より採用・要計測）
- out-of-game + focused 中、`Patches_Apply(PatchSite_OutOfGameFrame)` が毎フレーム
  ...
  ExpandStageMatrix）。[gameplay-runtime.md](../../../architecture/gameplay/runtime.md) §7 は
...
- dirty rect / GL upload skip（OPT-DR1..DR3）後も **CPU 側の clear + QPainter 再描画は
  残る**（[repo-architecture.md](../../../architecture/repository.md) OPT-DR3 節に明記済みの既知事実）。
...
4. mac の CGWarp 廃止（W1）は「raw アクティブ時のみ」。QCursor フォールバックの
   再センター（Accessibility 事故の再発防止ルール、[build.md](../../../development/build/overview.md)）は不変
5. 検証は3系統: Windows CI / mac ローカル / Linux VM（S20 手順は
   [linux-vm-build.md](../../../development/build/linux-vm.md) — マウス統合オフ必須）
...
| 項目 | 内容 |
|---|---|
| 対象ファイル | `EmuThread.cpp`（`MelonPrimeEmuThread*.inc` 断片として追加）、新規 `MelonPrimePerfProbe.h`、`tools/`（集計スクリプト） |
...
| 作業 | チェックリスト駆動 grep + 読解: (a) per-frame syscall（warp/GetAsyncKeyState相当/GetRawInputBuffer相当の非Win版）、(b) per-event の Qt オブジェクト生成、(c) atomic RMW vs load-first（P-48a 規律）、(d) epoch/dirty ゲートの漏れ（[melonprime-performance.md](../../../architecture/performance.md) の Invalidations 原則と突合）、(e) mac GL / software 両描画パスの per-frame 差 |
...
| 項目 | 内容 |
|---|---|
| 対象ファイル | [melonprime-performance.md](../../../architecture/performance.md)（追記）、コードは変更なし |
...
1. [melonprime-refactoring.md](../../../architecture/history/melonprime-refactoring.md) に「Round 10 (V5) — Measured」節を追記
   （変更ごとの実測値表。以後この文書の効果欄は実測値で書く、という運用転換を明記）
2. [melonprime-performance.md](../../../architecture/performance.md) に Phase 5 台帳と
...
手順: [`perf-baseline-procedure.md`](../../../development/performance/baseline-procedure.md)。
集計: `python3 tools/perf/summarize-melonprime-perf.py perf.log`
