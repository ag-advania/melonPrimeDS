...
**ステータス:** 完了（2026-07-02）
**前提:** [V1計画（2026-06-11完了）](melonprime-full-refactor-plan.md) /
[V2計画（2026-06-11完了）](melonprime-full-refactor-plan-v2.md) の後継。
...
- 当時のrules/completed分離は概ね健全だった。現行配置ではcompleted計画をこのarchiveに保持し、
  [`melonprime-refactoring.md`](../../../architecture/history/melonprime-refactoring.md)（統合履歴）へV3結果節を残す
- `docs/proposals/crosshair-smooth.md` の生死判定（実装済み/破棄済みかの明記）
...
3. CI に追加する監査は**既存スクリプトの実行**を基本とし、監査ロジックの新規実装で
   既存スクリプトと二重管理にしない
4. ビルドは `.\tools\build\windows\build-mingw.bat`（`--jobs 1` 厳守）。CMake 変更のないフェーズは
...
| # | 作業 |
|---|---|
| 1-1 | `docs/proposals/crosshair-smooth.md` の生死判定（実装済みなら completed へ、破棄なら理由を明記して削除） |
...
1. [MelonPrime refactoring history](../../../architecture/history/melonprime-refactoring.md) に「Structural Refactor V3 2026-07」節を追記
2. [Repository architecture](../../../architecture/repository.md) に HudGeometry 共有コアの所有点
   （コア → ランタイム/編集/ダイアログの3消費者の図）と「新 HUD 要素のプレビュー追加手順」を追記
3. CI 監査（Phase 0）の運用ルールを [build overview](../../../development/build/overview.md) に追記
