...
**先行文書:** [melonprime-metal-backend-plan.md](../../../../../plans/rendering/metal/backend-plan.md)（Phase 8/9 進行中）、
[SRP best-practices instructions](metal_flicker_black3d_srp_best_practices_instructions.md)
...
[melonprime-metal-backend-plan.md](../../../../../plans/rendering/metal/backend-plan.md) §0–1 と
[SRP 文書](metal_flicker_black3d_srp_best_practices_instructions.md) §2 を全文継承。特に:
...
| 7c | **フォーク固有修正の引き継ぎ（必須）**: [compute-renderer-mosaic-bug.md](../../../../investigations/rendering/compute-renderer-mosaic-bug.md) の **Fix D**（dispatch 分割 — GL は NVIDIA の Y/Z 65,535 制限対策。Metal はグリッド制限体系が異なるため上限を再調査し、同等の防御を移植）と **Fix E**（`MaxWorkTiles` 予算クランプ — バッファサイズ起因なので **Metal でもそのまま必要**。graceful degradation の設計を維持） |
