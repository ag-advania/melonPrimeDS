...
- MelonPrime UI labels use `MelonPrimeLocalization.h/.cpp` for English/Japanese display text selected by OS locale (`QLocale::system()`); this is display-only and does not translate config/TOML keys. melonDS-owned dialogs are localized via `LocalizeMelonDsDialog()` from `Window.cpp` — see [../archive/audits/upstream/non-melonprime-upstream-diff-2026-06-11.md](../archive/audits/upstream/non-melonprime-upstream-diff-2026-06-11.md) (do not patch upstream `.cpp` for UI text).
