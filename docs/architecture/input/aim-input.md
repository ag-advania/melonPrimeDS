...
`HandleMorphBallBoost()` consumes the previous-frame `altSteerDelta` (`CPlayer input +0x2A/+0x2C`) and the `CanTouchBoost` bit (`CPlayer +0x4C4`, bit 27). The game compares the squared steering magnitude with `0x1FA4`. The configured percentage is converted to an inverse-square threshold on config reload and cached in `m_morphBoostAssistThresholdSq`. The valid range is `0%`～`9000%`; `9000%` produces the minimum squared threshold `1`. Values below `100%` suppress a native swipe until the higher configured threshold is crossed; values above `100%` promote a below-native-threshold vector while preserving its direction. `0%` clears `CanTouchBoost` every applicable mouse-mode Morph Ball frame. This setting does not alter Stylus Mode, right-click R boost, or Shift auto-cycle. See [../../features/gameplay/morph-ball-boost-assist-sensitivity.md](../../features/gameplay/morph-ball-boost-assist-sensitivity.md).
