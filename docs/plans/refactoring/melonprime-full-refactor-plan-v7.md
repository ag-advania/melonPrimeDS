...
5. **ドキュメント・スクリプトの散乱**（SRP v3 の設計/進捗ドキュメント 6 本が
   `docs/features/` に平置き、i18n 一発物監査スクリプト約 20 本が `tools/` に堆積）
...
- `docs/features/` に SRP v3 の設計/進捗/監査ドキュメントが 6 本平置き
...
- `tools/` に i18n 一発物監査スクリプト約 20 本（`audit-melonprime-i18n-phase*.py`、
...
| 1-4 | i18n 一発物監査スクリプト（phase5a〜10 / qualityfix-pass1〜6 / he-coverage 等、参照データ未コミットのもの）を `tools/archive/i18n/` へ移動し、README に「手法記録・参照データ非コミットのため直接実行不可」と明記。恒久ゲート（`audit-melonprime-localization.py`）は現位置維持 | `tools/` |
...
| 3-3 | `melonprime-srp-performance-contract.md`（Load/Apply 境界）を feature note から binding contract へ昇格し、現行の `docs/architecture/srp-performance-contract.md` と短いperformance/threading ruleへ反映する |
...
2. SRP v3 の完了済み経緯ドキュメント（progress / completion-summary / step3 plan / site-d plan /
   aim audit / outcome-c note）を `docs/archive/features/srp-v3/`（新設）または
   `docs/archive/plans/` に移動し、features README を更新
...
| 1 | 小粒掃除（A1/A2 / API スイープ / skills 整理 / scatter budget是正） | 完了 | 2026-07-09 | A1: `PatchLifecycle::RestoreForEmuStop` の未使用 `romDetected` 引数を削除（呼び出し側1箇所も同時修正）。A2: `PlatformInput_WarpCursor` に Windows 到達時の規律違反リスクを明記するコメントを追加。**scatter budget regression の是正**: `audit-platform-scatter-budget.ps1` に行単位の `scatter-budget-exempt:` マーカー除外機構を追加し、`InputConfig.cpp` の macOS compute-renderer UI ゲート4箇所に注記（26→22 に復帰、CI予算22は不変）。スタレコメント修正: `DisableMacComputeVideoQualityButton` が参照していた存在しないファイル名 `melonprime-video-quality-macos.md` を実ファイル名 `melonprime_macos_compute_renderer_restriction.md` に修正。デッドコード再スイープ: TODO/FIXME/HACK/deprecated の実コード内出現は0（grepのfalse positiveのみ、翻訳文字列内の`toDouble`/`hacka`）。i18n一発物監査スクリプト14本（phase5a-10 × 7、qualityfix-pass1-6 × 6、he-coverage）を `tools/archive/i18n/` へ移動しREADME追加。`melonprime-i18n-handoff` skill の参照パスを更新。全ローカル監査 green + macOS ビルド green（新規warningなし）|
| 2 | HUD エディタ側パネルのテーブル駆動化（本丸） | 完了 | 2026-07-09 | V7-W1 の本丸。新規 `MelonPrimeHudEditorSidePanelRows.inc`（`MelonPrimeHudConfigOnScreenEdit.cpp` 単独所有の unity 断片）に `RowKind` 列挙 + `Row` 構造体 + 16種の名前付きファクトリ関数（RowBool/RowCombo/RowSpin/RowDoubleSpin/RowOpacity/RowLineEdit/RowColor/RowSubColor/RowColorOverlay/RowSeparator/RowSectionHeader/RowBuiltins/RowOffset/RowAlign3/RowOutline/RowGaugePosition）を追加し、14要素（HP/HPGauge/WeaponAmmo/WpnIcon/AmmoGauge/MatchStatus/Rank/TimeLeft/TimeLimit/BombLeft/BombIcon/WeaponInventory/Radar/Crosshair）全ての行データテーブルを定義。`MelonPrimeHudConfigOnScreenEdit::populateFromRowTable()` 汎用ディスパッチャ1関数が全要素を処理し、各 `populateXxx()` は1行のテーブル参照呼び出しに縮約（697→497行）。既存の `kProps*`（設定ダイアログ/オンスクリーン編集スキーマ）への統一はV2 Phase 2dの決定どおり不採用（行順・ラベルが意図的に異なるbespoke UX）。武器チントのループ（9要素配列）と crosshair のライフサイクル包装（`m_currentElem=-2`）はテーブル外に残置（計画の想定どおり）。**同値性の機械証明**: 移動前後で生成される全 add*/Row* 呼び出し列（関数名・label・key・range・combo items）を Python で再構成し diff — 14要素・178呼び出し全て完全一致（`archive/v7-phase2/` に検証記録保存）。副作用として発見: `generate-hud-prop-schema.py` の `parse_side_panel()` が `MelonPrimeHudConfigOnScreenEdit.cpp` のみをスキャンしテーブル移動後は side surface coverage が壊れる問題を検出・修正（新 `.inc` も合わせてスキャンし `RowXxx` 名を `addXxx` と同一視するようマッパーを拡張、combo item カウント正規表現も `QStringLiteral("` 限定から任意の引用符文字列に緩和）。再生成後、`MelonPrimeHudPropSchema.inc` は byte-identical（side 370 keys / missing defaults 0 / range drift 5、既存値と完全一致）、`MelonPrimeHudPropSchemaPhase2a.md` は origin タグ（`side:addSpinBox`→`side:RowSpin` 等）のみ更新。全ローカル監査 green（config-defaults / hud-key-parity -Strict / inc-ownership 56 / literal 1/1 / scatter 22/22 / color-dialog / srp-performance / localization）+ macOS ビルド green（新規warningなし）。目視スモーク（S9/S13）はROM実機が必要なため Phase 6 に送る |
| 3 | 節化・命名・ドキュメント昇格 | 完了 | 2026-07-09 | 3-1: `MelonPrime.h` に `class MelonPrimeCore` 直前のオリエンテーションバナー（public/privateの役割・不変条件へのリンク）と、public runtime state フィールド群の見出しコメントを追加。宣言順・メンバレイアウトは一切変更なし（`git diff -w` はコメント行のみ）。既存の cache-layout/aim-pipeline/transient-reset バナーは十分だったため追加変更せず。3-2: `RunFrameHook` の Site A/B/D/E 参照は SRP v3 側で既に完備を確認（抜けなし）。Site C（per-frame `OsdColor_ApplyOnce`、pattern B の明示的非対象）にラベル無しだったため `melonprime-srp-performance-contract.md` / `melonprime-srp-v3-completion-summary.md` への相互参照コメントを追加。3-3: `melonprime-srp-performance-contract.md` を feature note からbinding contractへ昇格（現行 `docs/architecture/srp-performance-contract.md`、CI監査 `audit-melonprime-srp-performance.ps1` 対象）。3-4: `repository.md` に「SRP v3 Unit Ownership」節を新設し、`RuntimeConfigSnapshot`/`AimConfigSnapshot`/`InputProjection`/`ScreenCursorPolicy`/`HudEditorFormBuilder`（+ V7 Phase 2 の SidePanelRows）/`PatchLifecycle`/`ColorDialogPrefs` の所有点表と意図的未分割リストを記載。全ローカル監査 green（inc-ownership 56 / literal 1/1 / scatter 22/22）+ macOS ビルド green（新規warningなし、diff はコメント/ドキュメントのみ） |
| 4 | 計測ゲート付き perf 残債（Windows 基準値待ち） | スキップ確定 | 2026-07-09 | Windows 実機がこの AI セッションから利用不可のため、`MELONPRIME_PERF=1` ソークを取得できない。計画 §4 の明示的フォールバック「届かない場合はこの Phase をスキップし『未計測のため見送り』と記録」に従い、V7-W3 の全候補（`Patches_Apply(OutOfGameFrame)` site 別 view / `OsdColor_ApplyOnce` edge 化 / `getScreenWidgetRect` キャッシュ / RAM read 予算 / HUD element cache）を**未計測のため実装見送り**として確定。凍結状態は解除しない — 将来 Windows `MELONPRIME_PERF=1` 基準値（+ OSD ON 条件）が取得され次第、本 Phase を再開できる。V6 進捗表の同一ゲート（Phase 0 Windows baseline pending）とリンクした状態を維持 |
| 5 | ドキュメント統合・クローズ処理 | 完了 | 2026-07-09 | `melonprime-refactoring.md` に「§27 Structural Refactor V7 / SRP v3 Integration 2026-07」節を追記（副次的に §25 の重複見出し番号を §26 へ修正）。SRP v3 の完了済み経緯ドキュメント6本（progress / completion-summary / step3 plan / site-d plan / aim audit / outcome-c note）を新設 `docs/archive/features/srp-v3/` へ移動し README 追加、`melonprime-patch-system.md` の相互参照リンクを更新、`melonprime-full-refactor-plan-v6.md` に V7 Phase 4 が同一 Windows ゲートに到達し正式スキップした旨のクロスリンクを追加。CLAUDE.md / rules README は Phase 3 で SRP contract 昇格時に更新済み（追加変更なし）。全てドキュメントのみの変更（ソース差分なし） |
| 6 | スモーク + CI 一括消化（最後） | 未着手 | — | ROM実機ゲームプレイ・Windows/Linux実機・GitHub Actions実行はこのAIセッションから実施不可。AI側で完結する検証のみ実施予定 |

### 計測により不採用（Rejected）

Windows 基準値が未取得のため、本 V7 サイクルでは「計測により不採用」の確定記入なし
（HUD element cache は V6 §6 の mac 実測で draw 非支配と判明済みだが、Windows 実測が
覆さない限り最終確定としない、という V6 の判断を継続）。

### 未計測のため見送り（Deferred — not Rejected）

Phase 4 は §4 のフォールバックに従い 2026-07-09 にスキップ確定。以下は「効果なしと
証明された」のではなく「計測できず着手しなかった」候補。Windows `MELONPRIME_PERF=1`
基準値が得られ次第、この計画を再開するか後継計画で再評価する:

- `Patches_Apply(PatchSite_OutOfGameFrame)` の site 別 constexpr view 化
- `OsdColor_ApplyOnce`（Site C）の edge/epoch 化
- `getScreenWidgetRect()` のレイアウト時キャッシュ化
- RAM read 予算監査（`ZoomStatus` / `DamageNotifyPurpleTick`）
- HUD element cache（V6 §6 の mac 実測は非支配を示唆するが Windows 未確認）
