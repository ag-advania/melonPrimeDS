...
| Native `VkSwapchainKHR` + `vkQueuePresentKHR`, present mode queried | `swapchain created ... Actual Present Mode=MAILBOX`, `first frame presented extent=256x384 presentMode=MAILBOX` |
| VSync on selects FIFO, VSync off selects MAILBOX | Two startup runs: `Requested VSync=on / ... Actual Present Mode=FIFO / Reason=FIFO is the specification-guaranteed VSync mode` and the MAILBOX form above |
| VSync toggles at runtime and the present mode really changes | The Video-settings VSync checkbox was driven three times through UI Automation on a live session; the log shows the swapchain rebuilt as `MAILBOX → FIFO → MAILBOX → FIFO`, each with its requested/actual/reason line |
| Validation layer clean | A dedicated build with `MELONDS_VULKAN_ENABLE_VALIDATION` (see §7.5) ran scales 1/4/16, VSync on, the 8-direction switch sweep and the window stress. `[Vulkan] validation layer enabled` in every run; **zero** `[Vulkan/validation]` and **zero** `[Vulkan/performance]` messages. The only debug-utils output was 28 `[Vulkan/general]` Vulkan *loader* notices about a duplicate OBS Studio capture layer installed on this machine — which also serves as the positive control that the messenger really was delivering messages to the log |
...
| Renderer switching, Vulkan↔Software round trips | `MELONPRIME_RENDERER_SWITCH_STRESS=3,0`, 40/40 switches, 21 Vulkan 3D-renderer initializations, 63 swapchain creations, no fallback or runtime failure |
...
| NVIDIA Reflex active on supported hardware | `NVIDIA Reflex (VK_NV_low_latency2): requested=yes supported=yes enabled=yes actual=active`, `mode=on lowLatencyMode=true lowLatencyBoost=false` |
| Anti-Lag 2 disables cleanly on non-AMD hardware, without failing the renderer | `AMD Radeon Anti-Lag 2 (VK_AMD_anti_lag): requested=yes supported=no enabled=no actual=disabled reason=this GPU does not expose VK_AMD_anti_lag` while Vulkan kept rendering |
