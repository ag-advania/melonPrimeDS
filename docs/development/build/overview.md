...
- `tools/ci/audits/audit-melonprime-thread-boundary.ps1 -Strict`
- `tools/codegen/hud/generate-hud-prop-schema.py` followed by `git diff --exit-code`
...
- Cursor clipping (`ClipCursor`) is Windows-only. On macOS the in-game aim path uses
  ...
  See [../../architecture/input/aim-input.md](../../architecture/input/aim-input.md) §10.
...
```bat
.\tools\build\windows\build-mingw.bat
```
...
```bat
.\tools\build\windows\build-mingw-existing.bat --tail 60
```
...
```bat
.\tools\build\windows\build-mingw.bat --verbose
.\tools\build\windows\build-mingw.bat --jobs 2
.\tools\build\windows\build-mingw.bat --tail 80
.\tools\build\windows\build-mingw-existing.bat --tail 80
```
...
Use `--verbose` for full compiler output when investigating an error; otherwise let the default 40-line tail keep transcript noise low. The skill source lives at [docs/development/build/windows-mingw.md](windows-mingw.md).

If a previous AI build timed out and the next run fails with `ninja: error: failed recompaction: Permission denied`, use the recovery steps in [docs/development/build/windows-mingw.md](windows-mingw.md#recovering-from-a-timed-out-build): inspect repo-scoped stale MSYS2/CMake/Ninja/compiler processes, stop only those tied to `melonPrimeDS`, then rerun the batch.
