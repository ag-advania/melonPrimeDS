...
```zsh
tools/perf/collect-perf-baseline.sh --label macos -- \
...
```bat
powershell -ExecutionPolicy Bypass -File .\tools\perf\collect-perf-baseline.ps1 `
  -Label windows `
...
Use the VM flow in `docs/development/build/linux-vm.md`. For FPS aim testing,
...
```bash
cd /mnt/mp
tools/perf/collect-perf-baseline.sh --label linux-vm -- \
...
```bash
python3 tools/perf/summarize-melonprime-perf.py artifacts/perf-baseline/<log>.log
```
...
- `docs/archive/plans/refactoring/melonprime-full-refactor-plan-v6.md` §8
- `docs/archive/plans/refactoring/melonprime-full-refactor-plan-v5.md` Phase 0 table

When one or more summaries are ready, apply them mechanically:

```bash
python3 tools/archive/perf/apply-perf-baseline.py \
...
```bash
python3 tools/perf/compare-perf-repro.py \
