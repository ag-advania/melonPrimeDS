...
`--check-budget` defaults to and requires at least 1000 calls; a smaller
explicit value is rejected because a short run is not certifiable. `--mode
controller` requires every joystick metric plus at least the minimum calls in
each metric, while `--mode keyboard` rejects any joystick metric calls.
`--mode raw` requires `snapshot` and `late_latch` stage populations to meet the
same minimum and requires the Raw capture-only marker. An old Raw report without
surfaced call counts is `legacy_unversioned` and is rejected unless
`--allow-legacy-raw-unversioned` is explicitly used for historical analysis.
