...
`docs/architecture/repository.md` "HUD Property Schema Ownership". The descriptor tables
...
After schema changes, run `tools/ci/audits/audit-hud-key-parity.ps1` and
...
Do not add these `.inc` files to `CMakeLists.txt`; they are included only through `MelonPrimeHudConfigOnScreenUnity.inc`.
