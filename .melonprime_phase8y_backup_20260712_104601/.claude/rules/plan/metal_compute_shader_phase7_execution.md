...
<!-- MELONPRIME_METAL_PHASE8N_PERSISTENT_UPLOAD_RING -->
## 2026-07-11 — Phase 8N persistent segmented upload ring

- Engine A/Bで毎frame生成していたsegmented upload MTLBufferを6-slot永続ringへ変更。
- SpriteConfig、ScanlineConfig、SpriteScanlineConfigのimmutable snapshot契約は維持。
- slotはcommand completion handlerで解放し、GPU使用中bufferへのCPU上書きを禁止。
- scale／device再構成時は新ringへ切替え、旧ringはin-flight callbackがshared ownershipで保持。
- capture不適格frameの判定をupload copyより前へ移動し、reject時の約1.7MB memcpyを回避。
- Software renderer、Metal 3D、visible-output ownership、scanline segment生成は変更しない。

<!-- MELONPRIME_METAL_PHASE8O_DIRECT_SNAPSHOT_RING -->
## 2026-07-11 — Phase 8O direct snapshot ring

- line 0でEngineごとの永続upload ring slotを予約し、192走査線のSpriteConfig／ScanlineConfig／SpriteScanlineConfigをslotへ直接記録。
- VBlank時に残っていた約1.7MB／Engineのframe-end memcpyを撤去。
- 中間SpriteScanlineSnapshotBufferを削除し、約1.7MB／Engineの重複shared bufferも撤去。
- 未送信slotは次frame開始時または失敗時に解放し、送信slotはcommand completion handlerへ所有権を移譲。
- ring exhaustion時はsnapshotを不完全として安全にrejectし、GPU使用中slotへ上書きしない。
- BG layer snapshot、segment hash、capture feedback判定、Metal 2D描画順序は維持。

<!-- MELONPRIME_METAL_PHASE8P_CHANGE_DRIVEN_SPRITE_CACHE -->
## 2026-07-11 — Phase 8P change-driven sprite cache

- OAM 1KB、OBJ関連DISPCNT、capture mapping、OBJ mosaic幅、VRAM maskを走査線ごとに比較。
- source変更時だけ128 OAMを正規化し、192走査線のactive maskを再生成。
- source不変ラインではtile／palette／capture／affine属性の再解析を行わず、active bitだけをOAM順に展開。
- OAMの途中更新は次のCaptureSpriteScanlineStateで検出し、そのラインから新しいcacheを使用。
- SpriteConfigCpuのcanonical zero tail、NumSprites、mosaic判定、capture texture種別は従来契約を維持。
- Software renderer、Metal 3D、visible-output ownershipは変更しない。

<!-- MELONPRIME_METAL_PHASE8Q_DEFAULT_VISIBLE_CUTOVER -->
## 2026-07-11 — Phase 8Q default Metal Compute visible cutover

- Video SettingsでMetal Compute Shaderを選択した場合、Compute final textureを環境変数なしで標準可視出力にする。
- 通常Compute frameではRasterReference.RenderFrameを停止し、既存のGPU-resident経路を使用する。
- CPU readback必須、AbortFrame、初期化不完全、span/tile/final slot不足時は同一frameでMetal RasterReferenceへfallbackする。
